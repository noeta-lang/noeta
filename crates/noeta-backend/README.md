...
Extracted from `noeta-eval` in M1 so the tree-walker (`noeta-eval`) and the bytecode VM (`noeta-vm`) are siblings — neither depends on the other, and the differential oracle compares their `RunResult`s, never their internal value models.
