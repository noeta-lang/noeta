...
enum ScopeAction {
    ...
    /// Replace the scope's publish token with a freshly minted one, printed **once**. Routine
    /// hygiene, and the way back from a lost or leaked token: authenticate with the scope's current
    /// token (`NOETA_REGISTRY_TOKEN`), or with the registry's admin token to recover a scope whose
    /// token is gone. The new token is generated by the registry, which keeps only its hash — so if
    /// you lose the output, rotate again. Ownership and the provenance policy are untouched.
    Rotate {
        /// The scope (`company`) whose publish token to replace.
        scope: String,
    },
}
