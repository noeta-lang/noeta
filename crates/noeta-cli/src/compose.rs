...
pub fn maybe_delegate(entry: &Path) -> Option<ExitCode> {
    ...
    match delegate(&resolved.native_crates) {
...
fn delegate(crates: &[NativeCrate]) -> Result<Never, String> {
...
fn compose_key(entries: &[Entry], toolchain: &ToolchainSource) -> String {
...
/// slice and hand the whole toolchain to `run_cli`. `Box::leak` is fine — the units live for the
/// process, exactly like the stock binary's statics.
fn shim_main_rs(entries: &[Entry]) -> String {
    let mut out = String::new();
    out.push_str(
    ...
    out.push_str("    noeta_cli::run_cli(Box::leak(units.into_boxed_slice()))\n}\n");
    out
}
