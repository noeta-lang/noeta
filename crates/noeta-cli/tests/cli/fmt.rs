...
/// **A `[directives]`-renamed text tier's body must survive `noeta fmt`** (parallel-path audit row 8).
///
...
/// tiers, but not the manifest's `[directives]` table, even though it already resolves the graph that
