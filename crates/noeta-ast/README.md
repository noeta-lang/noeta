...
`render_hint` — the shape of "how do I write this out?" for the one case where a value cannot answer for itself — is re-exported from here rather than defined here: it lives in `noeta-ext-abi`, beside the one JSON encoder its walk delegates to and the `MapKey` and `NativeValue` it describes, so the native seam can name it too. The re-export is what the checker and the IR use, since a hint describes a *static type* and this is the crate that spells those.

`conversion` names a declared conversion's body. A conversion is the ordered pair of types it goes between, and a method table has one slot per name, so each is named after its **counterpart** — `impl From<Source>` fills `from<Source>` and `impl To<Target>` fills `to<Target>` — always, whether the type declares one or five. The bare name is left free deliberately: it names a set, and a backed enum reserves its own `from` for the backing-value conversion. Everything that builds or resolves a method table asks that one function — the checker's signature registration, IR lowering, the bytecode compiler's prototype reservation, and the reflection manifest — so the four agree by construction rather than by four walks written to match.
...
`normalize` takes a `Normalization` for the two erasures a caller may also want: clearing the static text of tier bodies, and putting imports into a canonical order. The second sorts every contiguous run of `use` statements and the names inside each one, in every statement list the walk reaches — a module's top level, a function body, a `@test { … }` block alike — which is what lets a formatter that alphabetizes imports at any depth still compare equal to its input.
