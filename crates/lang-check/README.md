...
These are declaration-level checks (they never touch expression typing), so they stay gradual-safe: a correct program emits nothing, and the differential oracle is unaffected. Code generation (`@derive(...)`) and data attributes (`#[...]`) are two distinct decorators with two distinct sigils: `@derive` names derivable traits the compiler synthesizes; `#[...]` attaches a record as metadata (the manifest is M1.8b). The *behavior* the traits drive is wired in both backends (`lang-eval`, `lang-vm`): every operator is now trait-dispatched — `+ - * / ~` (`Add`/…, M1.8a), `==`/`!=` (`Equatable`), and `< <= > >=` (`Comparable`, via the built-in `Ordering` enum + `.compare()`). `@derive(Comparable)` synthesizes structural field-wise ordering for value objects, and `a[i]` index access dispatches through the `Index` trait (`get`), with built-in list element access (bounds-checked, `E0016`) as the fallback. The remaining derive codegen (`Display`/`ToJson`), the other protocols (`Length`/`Iterable`/…), the `Attribute`-trait gate on `#[...]` usage (with the manifest), and generics are the rest of M1.8b.
