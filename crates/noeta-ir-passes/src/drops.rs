...
use noeta_ir::{Arm, Block, ClassDef, Decl, EnumDef, Func, Program, Rvalue, Stmt, StructDef};
use noeta_span::Span;
...
/// `noeta_check::DestructorRelevance`; kept here so this crate needs no checker dependency — the
...
fn return_moved_out(value: &Option<noeta_ir::Atom>) -> VarSet {
    let mut out = VarSet::new();
    if let Some(noeta_ir::Atom::Var { name, .. }) = value {
...
fn tail_vars(block: &Block) -> VarSet {
    let mut out = VarSet::new();
    if let Some(noeta_ir::Atom::Var { name, .. }) = &block.tail {
        out.insert(name.clone());
    }
    out
}
...
fn block_exit_span(block: &Block) -> Span {
    if let Some(noeta_ir::Atom::Var { span, .. }) = &block.tail {
        return *span;
    }
...
fn for_pattern_names(pattern: &noeta_ir::ForPattern, out: &mut VarSet) {
    match pattern {
        noeta_ir::ForPattern::Single { name, .. } => {
            out.insert(name.clone());
        }
        // Lowering desugars a tuple for-pattern to a `Single` hidden var, so this is unreachable in
        // the IR; kept for totality.
        noeta_ir::ForPattern::Tuple { names, .. } => {
            for (name, _) in names {
                out.insert(name.clone());
            }
        }
...
fn pattern_names(pattern: &noeta_ir::Pattern, out: &mut VarSet) {
    match pattern {
        noeta_ir::Pattern::Binding { name, .. } => {
            out.insert(name.clone());
        }
        noeta_ir::Pattern::Variant { bindings, .. } => {
            for sub in bindings {
                pattern_names(sub, out);
            }
        }
        noeta_ir::Pattern::Tuple { elements, .. } => {
            for sub in elements {
                pattern_names(sub, out);
            }
        }
        noeta_ir::Pattern::Wildcard { .. }
        | noeta_ir::Pattern::Int { .. }
        | noeta_ir::Pattern::Str { .. }
        | noeta_ir::Pattern::Bool { .. }
        | noeta_ir::Pattern::IsType { .. } => {}
    }
}
