...
impl Checker {
    ...
    pub(crate) fn check_type_operand(
        &mut self,
        operand: &TypeOperand,
        env: &mut Env,
    ...
    pub(crate) fn synth_inner(&mut self, expr: &Expr, env: &mut Env) -> Type {
        match expr {
            ...
            // **The whole reflection surface, one arm.** It was thirteen, each computing its own
            // result while validating its own operand, and four of them had drifted into operand
            // contracts a type parameter could not reach. `synth_reflect` matches exhaustively on
            // [`ReflectKind`], so a fourteenth intrinsic does not compile until it says what it
            // evaluates to.
            Expr::Reflect {
                which,
                operand,
                span,
            } => self.synth_reflect(*which, operand, *span, env),
            Expr::Channel {
                elem,
                capacity,
                span: _,
