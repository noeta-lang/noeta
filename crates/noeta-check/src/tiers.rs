...
mod tests {
    ...
    fn extension_declarations_match_the_reflect_constants() {
        ...
        // The materialization shapes flow from the same declarations — resolved through the ONE
        // native-reflection lookup every consumer reads, so this asserts the shape the language
        // actually answers with rather than a second projection of it.
        let skip = noeta_ast::native_reflect::native_type_info(TEST_ATTR_SKIP).expect("Skip shape");
