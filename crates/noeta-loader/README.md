...
## Deferred sibling parses

A `parse_in` call costs ~200k instructions before it reads a byte — it rebuilds the whole combinator grammar every time (measured: an *empty* sibling file costs 197k, a 42-byte one 295k, flat across 2/6/12/24 siblings). Times the directory, that is what made a lone script pay for its neighbours: seven files in one directory cost `noeta run empty.noe` 5.86M instructions against 3.56M for the same file alone.

A **flat-scan** sibling — one with no derived module path, which is what a lone script's directory scan produces — has exactly one way to acquire an identity: its own `namespace` declaration. `apply_derived_paths` leaves such a file alone and the linking core admits a module to the resolution pool only if it declares a namespace, so a flat-scan sibling without one is in no pool, no `import_targets` list, no `project_roots` set, and no module map. It cannot be resolved, merged, or suggested, and its parse is deferred — recognized off the already-lexed token stream (every sibling is still *lexed*: that is ~12k a file, and a text tier declared in any one of them changes how all the others lex).

The one thing that parse could still have produced is a `BrokenModule`, which names the file as a hint on someone else's unresolved `use`. That is an error-path fact and it is bought back on the error path: a link that fails parses every deferred sibling and links again before reporting, so a failing program sees exactly the module set an eager parse would have handed it. A **package** sibling has a derived path and is genuinely resolvable, so it is never deferred.
