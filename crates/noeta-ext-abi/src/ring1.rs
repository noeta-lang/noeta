...
//!
//! Two marshalling generations meet here deliberately: [`Arg`]/[`Output`] predate
//! [`crate::NativeValue`]/[`crate::NativeOut`] and survive because the string surface's zero-alloc
//! `&str` projection has no `NativeValue` equivalent (its `Str` variant is owned). New extension
//! code should use the registry seam; this one is core-internal.
