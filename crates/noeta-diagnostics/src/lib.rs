...
pub enum DiagnosticCode {
    ...
    /// which is where this variant *reads* like it belongs, and exactly why it is down here
    /// instead. This enum is on the `.noeb` wire and postcard encodes a variant by declaration
    /// index, so a middle insertion silently renumbers everything after it.
