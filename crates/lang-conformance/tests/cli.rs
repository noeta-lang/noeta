//! End-to-end tests for the `lang-conformance` dev binary: the corpus runner, its JSON output, the
//! stage/file filters, and the unknown-stage error — driven through a real process so the CLI glue
//! and exit codes are exercised. (Moved here from `lang-cli` when the conformance harness was split
...
fn conformance() -> Command {
    Command::cargo_bin("lang-conformance").expect("the `lang-conformance` binary builds")
}
