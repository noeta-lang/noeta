...
//! it knows nothing about runtime values. The compiler (`noeta-compiler`) emits a [`Chunk`];
...
pub enum Op {
    ...
    /// dispatch and no override; and — since the ExtBundle→ExtTrait fold-in — every
    ...
    /// raises); the shared `noeta_ext_abi::mask_to_width` runs identically here and in the tree-walker.
    MaskWidth {
        dst: Reg,
        src: Reg,
        signed: bool,
...
pub struct Module {
    ...
    /// type's reflectable shape. Built from the AST by [`noeta_ast::reflect::build`] — the *same*
...
impl Module {
    ...
    pub fn attributes_for<'a>(
        &'a self,
        target: &'a str,
    ) -> impl Iterator<Item = &'a noeta_ast::reflect::AttributeRecord> {
