...
- **Emits:** three memoized, pass-through tracked queries forming the dependency graph `tokens(db) → ast(db) → bytecode(db)`, each a thin wrapper over `noeta_lexer::lex` / `noeta_parser::parse` / `noeta_compiler::compile`. The `LangDatabase` plus a `source_program(db, &Source)` constructor.
