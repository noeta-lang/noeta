...
- **Emits:** the `Op` opcode set (arithmetic/branching, `Call`/`Return`, the `MakeList`/`MakeMap`/iteration ops, and `CallBuiltin`/`CallMethod` for the collection prelude), the `Builtin`/`Method` dispatch tags, the `Chunk` (one function prototype: code + constant pool + precomputed diagnostics + parameter/register counts), the `Module` (the prototype table — proto 0 is the top-level program, the rest are functions/closures), and a disassembler (`Chunk::disassemble`/`Module::disassemble`) producing stable text for snapshot tests.
