...
## On-stack replacement compiles the loop's window, not the prototype

A prototype entered through a call promotes on its entry counter and gets a **whole-prototype** body. A prototype that gets hot at a loop **back-edge** — a top-level script's `while`, a long-running loop in a frame entered once — promotes through on-stack replacement instead, and there it gets a **region-scoped** body whose compiled region is the *OSR window*: the union of every loop extent containing the header, grown to a fixpoint, so an inner-loop back-edge still compiles the whole nest. Every pc outside the window is an ordinary bail.

The reason is Cranelift's whole-function register allocation. Growing the native region of a prototype is not free: nativizing one op in the cold tail of a top-level script (the trailing `Stringify` of `echo sum`) made its hot list-index loop spill a value that had lived in `%r14` — 35 extra stack reloads per loop traversal, +5.4% instructions retired, for a tail that runs once. Compiling the window on its own removes the competition, and it is what lets further ops become native without trading against the loops already there.

Both bodies coexist and the VM routes by entry pc: a re-entry whose pc falls inside a window takes that window's body, everything else the whole-prototype body. A prototype collects one window per distinct hot loop (up to `MAX_OSR_WINDOWS`, after which the whole-prototype body covers them all), and a prototype that only ever OSRs never pays for the whole-prototype compile at all. A window body is deliberately kept out of the table the native call helpers read — it has no fresh-frame entry, so nothing may call it as a callee.
