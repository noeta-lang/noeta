...
//! [`noeta_native::host::P2p`] capability with genuine peer delivery, the non-loopback backing the
...
//!
//! This crate depends only on `noeta-native` and the p2panda/tokio tree — never on `noeta-runtime`,
//! `noeta-stdlib`, or the extension crate — so the `para.p2p` extension can own the node without any
//! dependency cycle, and `noeta-runtime` sheds the whole iroh/QUIC tree.
...
/// An IO-kind [`StdError`](noeta_native::StdError) with `message` — the transport's single error
/// constructor (the node's operations are host effects; every failure surfaces as an `Io` error).
pub(crate) fn io_error(message: String) -> noeta_native::StdError {
    noeta_native::StdError {
        kind: noeta_native::ErrorKind::Io,
        message,
    }
}
