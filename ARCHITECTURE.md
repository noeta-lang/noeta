...
- **Precise reference counting via an ANF IR.** Memory management is compiled, not traced: `lang-ir` lowers to ANF and `lang-ir-passes` inserts drops at last-use and rewrites unique-owner mutations into in-place reuse. A cycle collector backstops reference cycles. See the [Memory Management](docs/Memory-Management.md) wiki page.
