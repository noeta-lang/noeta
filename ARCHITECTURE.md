...
| Crate | Role |
|---|---|
| `noeta-ext-abi` | The extension **ABI**: the `Host` supertrait (the capability traits — filesystem, rng, clock, env, console, os, entropy, ids, network, tracing, metrics, logging — plus the `P2pProvider` policy seam, through which a host optionally offers peer networking, and `Cancellable`, the cancel flag/wake pair a blocking leaf reads), `ExtModule`/`ExtType`/`ExtFn` registration, the assembled `Registry` + the process-default slot (`install`/`single_registry_process`), and the neutral `NativeValue`/`PackedView` marshalling seam. What third-party native packages link against — and the **only** extension crate the front-end (`noeta-check`/`noeta-loader`/`noeta-compiler`/`noeta-ir`/`noeta-db`) links: they consume the registry as *data* seeded by the assembling driver, so a stdlib method-body edit rebuilds none of them (audit-6 F2). |
