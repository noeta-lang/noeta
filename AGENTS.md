...
In progress (M1, see `plans/m1/`): the layered stdlib (`lang-stdlib`) and the rest of Thrust C. The salsa query graph (`lang-db`, M1.1), the gradual type checker (`lang-types`/`lang-check`, M1.7), the trait system (M1.8), and multi-file modules (`lang-loader`, M1.9.1 + M1.9.2: `pub` visibility, E0019/E0013/E0020) have landed; the module graph as salsa queries (M1.9.3) is next. Deferred to later milestones (do **not** stub now): `runtime`, `server`, `lsp`.
