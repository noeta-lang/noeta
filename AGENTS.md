...
| `noeta-ide` | Shared IDE engine (hover/def/refs/outline/call+role graph) over the salsa db; reused by the LSP and MCP servers. Cursor- and buffer-shaped only — the project model lives in `noeta-project`, which this crate consumes and re-exports. |
