...
| `noeta-project` | The **project model**: `project_check` (the one answer `noeta check`, the LSP's `workspace/diagnostic` and the MCP `check` tool give), the entry/pool decomposition (`entry_pool`/`pool_modules`/`noe_files`), and the shared disk-backed salsa `Workspace` construction. Sits between `noeta-db` and `noeta-ide` so `noeta check` needs no editor crate. **Rule:** project-model questions — module paths, package roots, URI→path, dependency selection for a `--target`, the sibling pool an entry links against — are *delegated* to `noeta-loader`/`noeta-pm`, never re-derived here. Six `check`-vs-`run` divergences were re-derivations that drifted, and the failure direction is the bad one: `check` exiting 0 on a tree `run` refuses. |
| `noeta-ide` | Shared IDE engine (hover/def/refs/outline/call+role graph) over the salsa db; reused by the LSP and MCP servers. Cursor- and buffer-shaped only — the project model lives in `noeta-project`, which this crate consumes by name and deliberately does **not** re-export: reach it as `noeta_project::…`, never through `noeta_ide::`. |
...
- **The CI toolchain is pinned to `@1.97.0`** (in every `ci.yml` job + `release.yml`; miri stays nightly). Clippy lints are version-sensitive, so lint against 1.97.0 locally — a floating `@stable` will silently add lints that only surface as a red CI you didn't cause. `scripts/gate.sh` selects `+1.97.0` for you when it is installed.
- **Read the exit code, never a pipe's.** `cargo clippy … | tail` reports `tail`'s status, so a broken tree passes a local "check" — that is not hypothetical, it is how a `clippy -D warnings` violation once reached `main` under a green-looking local run. If you need to see the output, redirect it to a file and read the file.
- Both of the above are what `scripts/gate.sh` exists to do for you; see *Before You're Done*.
...
> [!IMPORTANT]
> **Performance regressions are gated on INSTRUCTIONS RETIRED, not wall-clock — `scripts/perf-ratchet.sh`, run by `scripts/gate.sh` at the merge tier.** Wall-clock cannot gate on this box: several agent builds run concurrently, load 6–13 is normal, and a whole field run of wall-clock benchmarks inflates ~2x together, so a 7% regression is indistinguishable from a busy afternoon. That is not theoretical — roughly 1,800 commits landed a **2x startup regression and a 7–11% interpreter regression** and nothing in the tree noticed. Instructions retired repeat to 0.001–0.08% under exactly that load, so the ratchet pins five rows (process init, `run --no-cache`, an interpreter arithmetic loop, a map workload, and the same loop under tier 1) against `tests/perf/baseline.txt`, and asserts **which engine each row ran on** — equal JIT and interpreter numbers mean the JIT never ran, which is a finding, not a footnote.
>
> Expect to re-baseline: perf work lands continuously, and an improvement past the band FAILS on purpose so the ratchet holds the new floor. `scripts/perf-ratchet.sh --record` rewrites the baseline; **read the diff's sign before committing it** — down is an improvement worth keeping, up needs a sentence saying which change bought it and what for. Build the binary with the gate's pinned toolchain first (`cargo +1.97.0 build --release -p noeta-cli`): the baseline records cpu, libc *and* `rustc -V`, because two rustc patch releases inline differently and on this codebase that is worth percent. A machine or compiler mismatch makes the gate report **CANNOT MEASURE (exit 2)**, never a pass and never an invented regression. Everything above — including the measured variance behind each tolerance — is documented at the top of the script.
...
>
> **Order the prune: remove the worktree *first*, then the target dir — and re-check liveness immediately before deleting, not before the merge.** A merge and a prune are seconds apart for you and minutes apart for the agent still finishing its gate run. Reclaiming a target dir out from under a live `cargo test` does two things at once: it corrupts that run (19 `noeta-cli` tests failed with `cannot read …/tmp/init_runnable/src/main.noe`, a result that looks exactly like the fixture-path bug and wasted a report explaining it), and it leaves **residue** — the dying cargo keeps writing while `rm -rf` deletes around it, stranding 425 MB that the "reclaim" did not reclaim. Removing the worktree first makes the cargo fail fast instead of racing; if you must delete the target dir while a process holds it, kill that cargo first.
>
...
> | `scripts/gate.sh` | + workspace suite & oracles, lean-CLI + feature shapes, doc samples, JIT differentials, the `#[ignore]`d real-socket hot-reload suites, the instructions-retired perf ratchet | ~20 min | 42 min |
> | `scripts/gate.sh --full` | + wasm portability/differential, miri, editor tooling | +2 min and up | — |
>
> `--quick` is the inner loop, the default is the merge gate, `--full` is for a release tag. The merge tier's perf group adds a third link of the CLI (a `--release` one, ~6 min cold here) and then measures in seconds; it SKIPs — loudly, naming the reason and the fix — without `perf`, when `perf_event_paranoid` forbids counting, or when the baseline was recorded on a different machine or compiler. `--full`'s miri and editor legs are cheap (1m12s and 4s); its wasm legs SKIP without `wasmtime` on PATH and without the wasm targets installed *for the gating toolchain*, and are the expensive part when they do run. Useful flags: `--list` (print the plan), `--only <substring>` (one group), `--install-hook` (opt-in `pre-push` hook running `--quick` — never installed for you, and **not recommended here**: hooks live in the *common* git dir, so installing one changes every worktree of this repo including other agents', and no hook fires on the fast-forward merge into `main` that is the moment we actually care about). Honor `CARGO_TARGET_DIR`/`CARGO_BUILD_JOBS` as everywhere else — the script respects them and sets neither.
