...
| `noeta-check` | The gradual type checker (`check(&Program) -> Vec<Diagnostic>`), the `checked` query's body. A shared front-end run upstream of both backends: exhaustiveness (E0011), `?`-typing and the `?` **position rule** over both halves — an `Option` needs a `?T` return, a `Result` needs a `Result<T, E>` one (E0012; a deferring return still defers, and an `Err` that reaches the top aborts at runtime as E0069) — arithmetic mismatch (E0007), unknown-type resolution (E0013, lit up once modules give type names referents), trait/derive validation (E0014 unknown trait, E0015 invalid impl), and data-attribute validation (E0017 invalid attribute). |
