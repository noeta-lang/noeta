...
**Status: M0–M5 done; M6 next.** `noeta mcp` runs over stdio (`rmcp` 2.1) with ~21 tools across all four pillars — Ground (`docs_search`/`docs_get`/`examples_find`/`explain_diagnostic`/`stdlib_api` + docs as MCP resources), Understand (`check`/`type_at`/`symbols`/`definition`/`references`/`completions`/`signature`), Introspect (`ast`/`bytecode`/`pipeline`/`module_graph`/`reflect`), Execute (`run`/`eval`/`test`, sandboxed with always-on liveness limits) — plus `format`. Navigation rides the `noeta-ide` engine shared with the LSP. Every slice gated by a real client⇄server round-trip. This is the third leg of the editor-tooling story —
