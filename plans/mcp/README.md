...
**Status: M0–M4 done; M5 next.** `noeta mcp` runs over stdio (`rmcp` 2.1) with five tools — `check` (typed diagnostics for inline/on-disk Noeta) plus the Ground pillar `docs_search`/`docs_get`/`examples_find`/`explain_diagnostic` over the embedded docs+example corpus — and exposes docs as MCP resources. Verified by a real client⇄server round-trip. This is the third leg of the editor-tooling story —
