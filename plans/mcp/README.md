...
**Status: ARC COMPLETE — M0–M7 all shipped.** `noeta mcp` runs over stdio (`rmcp` 2.1) with ~26 tools across all four pillars — Ground (`docs_search`/`docs_get`/`examples_find`/`explain_diagnostic`/`stdlib_api` + docs as MCP resources), Understand (`check`/`type_at`/`symbols`/`definition`/`references`/`completions`/`signature`), Introspect (`ast`/`bytecode`/`pipeline`/`module_graph`/`reflect`), Execute (`run`/`eval`/`test` + `debug_start`/`debug_inspect`/`debug_step`/`debug_eval`/`debug_stop` interactive sessions, sandboxed with always-on liveness limits) — plus `format`. Navigation rides the `noeta-ide` engine shared with the LSP. Every slice gated by a real client⇄server round-trip. This is the third leg of the editor-tooling story —
...
| **M4** ✅ | Execute — run / eval / test + `format` | "Run this and tell me what happened"; clean it up | `run` compiles via the salsa `linked_bytecode` query and runs the module tier-0 against the sandbox (default) or real host; **always-on liveness limits** ride the VM's own per-op `Debugger` seam — a `LimitDebugger` counts instructions and samples a wall-clock deadline, returning `DebugAction::Terminate` to stop *in-VM* (real bound, clean teardown, no leaked thread) — plus a stdout byte cap (decision #5). `eval` rides `VmSession` (value + REPL-`:type`); `test` activates the `@test` tier over the linked program and runs each case (incl. `#[Data]`/`#[Skip]`/`#[Name]`/`#[Group]` filter) as a `VmSession` entry. `format` wraps the ready `noeta fmt` arc's `noeta_fmt::format_source` (decision #6). Gotchas: a Noeta program is its **top-level statements** (no auto-invoked `main`); `@test` fns need an explicit `: void`. New deps: noeta-vm/check/parser/runtime/fmt. Slice gate = a `run` duplex fixture; 44 mcp tests green. |
| **M5** ✅ | Understand (full) — defs / refs / completions / signature | Precise cross-symbol navigation | Landed in two commits. **M5a**: `noeta-ide` extracted from `noeta-lsp` (decision #3) — `DocumentStore` + all seven analysis modules (offsets/resolve/completion/signature/symbols/inlay/semtokens) moved to a wire-protocol-free crate with its own field-compatible positional types (`Position`/`Range`/`TextEdit`/`SemanticToken`/`DocumentSymbol`/`SymbolKind`); `noeta-lsp` is now a thin tower-lsp adapter (Backend + mechanical conversions + encoding negotiation, deps cut to `noeta-ide`+`noeta-diagnostics`+transport); the ~107 DocumentStore fixtures moved with the engine, assertions unchanged. **M5b**: `navigate.rs` adds the four tools over a fresh per-call store — `file` entries get full workspace behavior (siblings + dependency packages → cross-file definition), inline `source` is a lone document; sites address by `symbol` (probing every whole-word occurrence until one resolves — the first is often the declaration, which the use-index can't anchor) or 1-based line/column; completions cap at 100 (`truncated`); definition answers carry the target's source line. Slice gate = a `definition` duplex fixture. |
| **M6** ✅ | Execute — debug sessions | Programmatic breakpoint / inspect / eval / step | Landed in three commits. **M6a**: the protocol-neutral debug-session support moved from noeta-dap into `noeta_vm::debug` (decision #4's shared spot, grown to its natural size) — `resolve_breakpoints`, line-granular `StepMode`/`StepState`, owned `PausedState`/`capture`, `frame_param_names`; the closure-wrapping fragment check became `SessionChecker::check_closure_fragment` (the VM stays checker-free); noeta-dap re-adapted, its 19 fixtures unchanged. **M6b**: the nested debug-console eval is now BOUNDED in-VM — an `EvalBudget` debugger armed around `run_installed_fragment` (5 s / 500 M steps → the fragment aborts, the paused program survives); fixed the pre-existing hang for DAP watch expressions too. **M6c**: the five `debug_*` tools over a session registry — one run thread per session, `McpDebugger` blocks on a command channel and publishes rendered state through a condvar cell; resumes wait (bounded) for the next stop; every resume carries a liveness budget so a runaway `continue` lands in an inspectable `limit` pause; `debug_eval` type-checks via C3 before running. Sandbox default, `real` opt-in, 8-session cap. Gate = start→eval→stop duplex fixture. |
| **M7** ✅ | Wire auto-registration | The extension registers the MCP server; other clients documented | Shipped: the extension registers `noeta mcp` via `vscode.lm.registerMcpServerDefinitionProvider` + `McpStdioServerDefinition` (guarded for hosts without the API; `engines.vscode` → ^1.101.0, extension 0.5.0, provider id `noeta.mcp` contributed in `mcpServerDefinitionProviders`); reuses `noetaCommand()`/`noeta.server.path`. `docs/Editor-and-AI-Tooling.md` rewritten from roadmap to the shipped agent surface — full ~26-tool catalog by pillar and the `claude mcp add noeta -- noeta mcp` snippet; `Home.md` status line de-staled (decision #7). |
