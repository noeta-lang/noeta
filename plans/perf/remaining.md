...
| **P-SHARE** | 🔀 **Picked up in the [parallel-seams arc](parallel-seams/README.md) (P-PAR S1+S2).** Zero-copy borrow-share for isolate args — wire the built-but-unused I.3 `SharedRegion` into the real spawn path | isolates I.3/I.4 | `Rc<Shape>`→`Arc<Shape>` (a prereq slice) | Medium–large |
...
| **P-POLISH** | 🔀 Stall-yield sleep-spin **picked up in the [parallel-seams arc](parallel-seams/README.md) (P-PAR S3)**; remaining micro-item: `send` marshals only on a race today (already cheap) | isolates I.4b/c | — | Small |
...
| **P-CALL** | **Native call frames.** ⚠️ **Mostly DROPPED after measurement.** S1 (layout lock `2ac6d4f`) + S2 (inline hot return path `775f96f`, **+2.4% fib**) done; S2 was measurement-first and **disproved the premise** — removing a bare hot-path helper call is only ~2.4%, so the per-call cost is the frame-setup **work** (~25 of ~30 ns/call), NOT the helper round-trip. S3/S4 (native `Vec`-header inlining) DROPPED: ~2–3% for real corruption risk. **Real fib lever moved to: shrink `Frame`, cheaper register-window init, or the fixed-`int` heap-aware tax (P-BITS Tier W).** See [`p-native-call-frames.md`](p-native-call-frames.md). | 2026-07-04 x-lang bench + fib dump | — (S1–S2 done) | ~~Large~~ → capped at S2 |
