...
| Item | Source / trigger |
|---|---|
| **bug — `?` on a `Result` inside a `void` function silently discards the error.** The `Option` half of the position rule exists (`check_try_option`: `fn f(): void { xs.first()? }` is `E0012`, "`?` on an `Option` early-returns `none`, but this function returns `void`"); the `Result` half does not — `check_try_error` returns early with "no declared `Result` context" whenever the return is not a `Result`, so there is no position check at all. Measured: `async fn run(): void { echo "before"; r = client.get("https://nonexistent.invalid/x")?; echo "after" }` prints `before`, does **not** print `after`, emits no diagnostic, and exits 0 — the function stops at the `?` and the caller carries on. Identical before and after the stream-status work, so pre-existing. It is the same invisible-failure class as the discarded stream status, and it undercuts it: a client written in this shape cannot see a transport failure either. Fix is the symmetric rule (`?` on a `Result` needs a `Result`-returning, `dyn`, or deferring context); blast radius measured at **6** corpus cases (`async/http_async.noe` + the five `std/http_stream_*.noe`) plus the `docs/Concurrency.md` sample, all written `async fn run(): void { …? }`, so it is a language-acceptance change wanting a deliberate call: reject the `?`, or make a discarded `Err` an abort | stream-status session (2026-07). Trigger: pick it up as its own slice — **active** |
