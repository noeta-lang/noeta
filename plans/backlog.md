...
| **`Error`-trait ergonomics: `From`-conversions + `?` interop** — auto-converting an `Err` payload across error types at a `?` boundary (`Result<T, JsonError>` → `Result<T, AppError>` via a declared conversion), so multi-source pipelines don't hand-map errors. Deliberately NOT shipped with the `Error` trait slice (kept minimal: one trait, one method) | error-machinery arc (2026-07). Trigger: a real program juggling ≥2 error types through one `?` chain |
| **`@derive(Error)` via `Display`** — a mechanical derive forwarding `message()` through an existing `to_string()` (or a `via:` field). Skipped in the trait slice: an `impl Error` is a single method, so the derive saves little until error wrapper types are common | error-machinery arc (2026-07). Trigger: demand from wrapper-heavy error hierarchies |
