...
| **bug** — **a `use` inside a tier block resolves an attribute's name but does not qualify it, so the attribute silently does nothing.** `@test { use std.test.{Skip}  #[Skip("…")] fn f(text: string): string {…} }` raises no diagnostic and does **not** skip: `f` runs as a test and fails on its missing argument. The same `use` at top level works, as does `#[std.test.Skip("…")]` inline, and no import at all is the clean "`Skip` cannot be used as an attribute" error — so the block-scoped form is the one spelling that fails quietly. Ordinary code through a block-scoped `use` is fine (`use std.math` in a `@test` block, then `math.abs(-3)`). Cause is pass order: the runner matches the qualified `std.test.Skip` (`reflect::TEST_ATTR_SKIP`), `noeta-loader`'s qualifier builds its rename map from a module's *top-level* imports, and a tier block's `use` only reaches top level after `activate_tiers` runs. Fix is either to collect a tier block's `use`s into the unit map before qualification (they are dropped with the block when the tier is inactive) or to diagnose a block-scoped import that an attribute name depends on | found writing para/ai's `#[Tool]` test fixtures (2026-07-28) *(active)* — a silently-ignored `#[Skip]` runs a fixture as a test |
...
| **bug** — **`noeta mcp` gives its tokio workers no `thread_stack_size`, so a debug build overflows the stack on ordinary real-world files.** `run_stdio` (`crates/noeta-mcp/src/lib.rs`) builds `Builder::new_multi_thread()` with no stack size, so a worker gets tokio's 2 MB default; against para/ai's 36–75 KB modules every file-based tool (`check`, `symbols`, `module_graph`, `reflect`) aborts the whole server with `thread 'tokio-rt-worker' has overflowed its stack`, killing the client's session mid-request. A release build of the same tools on the same files is fine, and a small two-package probe is fine in debug too — so it is frame size × recursion depth, not a shape the analyzer cannot handle. An agent developing against a debug toolchain reads this as "my program is broken". Fix: set an explicit `thread_stack_size` on the runtime (the LSP/DAP `run_stdio`s want the same audit), and/or grow the recursive walk's stack on demand | found running `reflect` over para/ai from a debug `noeta` (2026-07-28) *(active)* |
