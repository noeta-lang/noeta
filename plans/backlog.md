...
| **`std.session`** — the stateless signed-cookie codec (S2) and the `Session` request/response pair (S3), designed in `plans/http-sessions/` | cookie arc S1 tail. *(active)* — S1 shipped the `Cookie` surface it builds on |
