...
| **Return-position inference through `?`** — `o: Order = load(text)?` inferring `T` by inverting the `Result` wrapper at a check-mode `Try`. The direct binding (`r: Result<Order, E> = load(text)`) infers today (F2c seeds `T` from the expected return); through-`?` needs the expectation to flow into the `Try` operand as `Result<expected, ?>`, which check mode does not model | poly-values arc (2026-07). Trigger: real code hitting the turbofish-required `?` case often |
| **Forwarding gaps** — type-param forwarding into call-site-typed positions is top-level-generic-fn, bare-parameter only: forwarding from a generic CLASS's methods (the class's `T`), from nested `fn`s, composite forwards (`List<T>` in a `::<...>` position), and `roles_of::<T>`/`from_bytes::<T>` forwarding are all clean checker errors today (E0058/E0023). Each is an additive extension of the hidden-slot machinery (`TypeArgInfo` table + `$ty` params) | poly-values arc F2b (2026-07). Trigger: a real generic service type wanting per-`T` decode in a method |
| **Generic methods** — methods carry no type parameters of their own (a method is generic over its class's parameters only), so the `f::<T>(args)` turbofish covers free functions. Adding per-method `<T>` is a parser + collect + dispatch extension | poly-values arc F2a (2026-07). Trigger: first API that wants a method-scoped type parameter |
