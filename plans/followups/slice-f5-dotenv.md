...
- **In (follow-up pass — landed):**
  ...
  - **Shell-style default/alternate operators** inside `${...}`: `${VAR:-word}` / `${VAR:=word}` (word if unset or empty), `${VAR-word}` (word if unset), `${VAR:+word}` / `${VAR+word}` (word if set); the `word` is itself interpolated, so defaults may nest (`${A:-${B}}`). The resolver distinguishes unset (`None`) from empty (`Some("")`) for `:-` vs `-`.
- **Out (permanent):**
  ...
  - **`${VAR:?error}`** — the error-if-unset shell form; the parser never raises (lenient dotenv), so this is intentionally omitted.
...
**Follow-up pass (interpolation + multi-line) — landed.** `${VAR}` / `$VAR` interpolation (ambient-wins precedence via `parse_dotenv_with_env`, `\$` literal, single-quotes exempt) + multi-line double/single-quoted values. Single expansion pass handles C-escapes and interpolation together so `\$` is unambiguous; `read_value`/`read_multiline` assemble line-spanning quoted values. `env.load` now passes the ambient env as the interpolation base *and* the whole-key overlay. +6 unit tests, +2 conformance cases (`std/env_dotenv_interpolation`, `std/env_dotenv_multiline`).

**Shell-default pass — landed.** `${VAR:-word}` / `${VAR:=word}` / `${VAR-word}` / `${VAR:+word}` / `${VAR+word}`, with `word` itself interpolated and nestable (`${A:-${B}}`). The resolver now returns `Option<String>` (unset vs empty), `read_var_ref` does balanced-brace matching so nested `${...}` in a default survives, and `resolve_var` splits name + operator + word. +4 unit tests (22 total), +1 conformance case (`std/env_dotenv_defaults`). Corpus + differential + `cargo test --workspace` + clippy `-D warnings` all green. **Nothing left open** (only the raise-on-unset `${VAR:?err}` form is intentionally omitted — the parser never raises).
