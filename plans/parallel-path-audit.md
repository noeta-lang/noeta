...
**Closed, 2026-08-01: the fleet's wake now reaches every worker, and it was never the wake.** The row recorded an idle `--parallel N` fleet applying a deposited swap in only one or two of its workers whatever N was (0 stale responses at N=1 and 2, 1–2 at N=3, 3–4 at N=5, re-measured), and pointed at where an idle worker holding a pre-bound listener blocks. It does not block anywhere the wake cannot reach: instrumenting the park showed every worker roused by `notify_waiters`, and then losing the swap on the *far side* of the wake. `HotChannel::drain` took the queue with `try_lock` and returned `None` on contention, on the reasoning that "the next tick retries, and the cursor makes the retry lossless" — the second half is true, the first is not. A wake buys an idle worker exactly **one** scheduler iteration: it runs the safepoint, finds no traffic, and parks again on its accept. So the losers of a race that N simultaneous wakes make certain dropped the swap until a request arrived, and the stray `notify_one` permit bought exactly one of them a second iteration — which is the whole of the "exactly two". Fixed by blocking on the lock, affordable because the queue now owns each plan behind an `Arc` and a drain clones pointers rather than deep-copying the fragment AST per worker (which was both what made the section long enough to lose and N whole-program AST copies per swap). Zero stale at N = 1, 2, 3 and 5 across four runs. This is the third proposal in this document to be wrong until it was built and measured, and the second whose measurement (per-consumer `Notify`s changing nothing) was *right* and read as pointing the wrong way.

**Gates.** `cmd::serve::tests::every_step_of_the_hot_install_is_performed_in_exactly_one_place` censuses the serve/watch pair for the single call performing each step, so a second install fails the build naming the step; `parallel_hot::an_idle_swap_reaches_every_worker_before_the_next_request` (`#[ignore]`d, `hot-e2e.sh`, count bumped 1 → 2) runs one edit against the single worker and fleets of 1, 2, 3 and 5, and holds every shape to **zero** stale responses. It was a bound (`stale < workers`) while the number was unknown, and the bug satisfied the bound at every N — which is how it stayed invisible. Dropping `set_wake` fails it, and so does restoring the `try_lock` drain.
...
**CLOSED 2026-08-01, `fix/manifest-order`.** The differ change was the right one, but only half of it, and the note above names the half that is easy to miss: "re-registering the manifest in the new order" is not something the differ can do alone, because `ReflectionInfo::accumulate` pinned each incoming group to the slot it already occupied and would have undone the reorder the fragment expressed. The two halves are `diff_programs` comparing the declaration **sequence** and handing the whole region to the fragment when it moved, and `accumulate` handing the base's slots out **in the fragment's order** rather than per group. Together they mean the order is *carried* — by the fragment's own sequence — where before it was implied by the session's insertion history; no field beside the fragment describes it, so nothing can drift from it.

Supersede-in-place survives by construction rather than by a second code path: a body edit's fragment carries one group, one group has one slot, and one slot cannot move anything. Mutation-verified in both directions — restoring the per-group anchor fails the four reorder cases while the two in-place pins stay green, and removing the slots entirely fails exactly those two pins.

Three things the audit's write-up did not have:

- **The same family covers an insertion.** A declaration added *between* two existing ones appended where a cold start has it in the middle — the merge had no slot for a name the session had never seen. It is fixed by the same rule (a new key buffers onto the next slot), and it is the commoner edit of the two.
- **A standalone `impl Trait for T` declares its methods' manifest rows from where the impl sits.** A reordered region that re-registered the type without the impl left `T.method`'s rows in the old order — dispatch was never affected, only the listing. The impl rides along when the region moved, which is safe because any edit to one is an `ImplChanged` restart, so the one in hand is byte-identical to the one running.
- **The impact engine had a matching false negative.** A reorder changes no definition, so `changed`/`added`/`removed` are empty and `noeta test --watch` reran nothing — for an edit that can change what a test prints. `SwapPlan::declarations_reordered` (bookkeeping, like `rerun_top_level`) is the bit it reads to seed the moved region.

Five hand-written cases in `crates/noeta-vm/tests/hotswap.rs` (reorder → manifest, reorder → role index, insertion, standalone impl, and the append that must stay on the cheap path). The corpus is unchanged in every count — 296 / 277 / 674 / 328 / 633, `KNOWN_DIVERGENCES` still `&[]` — which is the point: none of its generators moves a declaration, so none of them can see this, and a sixth generator that rotates the region would be the way to put it under the corpus rather than under five hand-written programs.
