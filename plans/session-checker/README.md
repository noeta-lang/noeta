...
**Status: ARC COMPLETE — C0–C5 shipped and checking is the REPL DEFAULT (`--no-check` / `:check off` opt out).** The follow-on `plans/repl-on-vm` §"Incremental
...
| **C3** ✅ | Debug-console checking (runs on the DAP worker in `DapDebugger::wait` — the VM stays checker-free; the check wrapper is a bare closure expression, seen BEFORE the U2 mut-promotion rewrite; setVariable values check too; hover skips) | The launch's `check_all` becomes session-flavored (keep the `Checker` alive, exactly as T3 kept the `ModuleCompiler`); console fragments check before running — `E0xxx` at the console. Frame-local wrapper params bind as `dyn`/Unknown first (no false positives; runtime `TypeRepr` → checker-type refinement is a follow-on). | The console analogue of C2, riding the same `check_entry`. |
| **C4** ✅(oracle) | Oracle + default decision (five (code,span)-exact scripts in noeta-check/tests/session_oracle.rs, restricted to the entry-under-check's SourceId — the whole-program check sees later entries' declarations, a session cannot; default-on = open user decision) | Conformance: a checked-session differential — per-entry `check_entry` diagnostics vs a **whole-program re-check of the accumulated source** (the sketch's oracle). Then decide with the user whether `--check` becomes the default. | The behavior change users feel (entries can be rejected) stays opt-in until proven. |
| **C5** ✅ | Checked REPL codegen (+ default flip) | `SessionCompiler::extend_checked(entry, &Sites)`: file-pipeline lowering + precise relevance/reachability; `type_of([1,2])` prints `Type.List(Type.Int)` at the prompt. | Soundness gate: `precise_codegen` true only while EVERY entry was checked — `:check off` clears it permanently (`:reset` earns it back); conservative codegen is the always-sound fallback (mixed sessions, `:type`). map-packed pairs accumulate on the session so every snapshot carries every earlier entry's pairs. Coverage note: the session differential covers the checkerless path; checked lowering is the corpus-covered file path + targeted CLI tests. |
