...
- **Spread of a non-list display-concatenates instead of erroring.** `[..notAList]` falls through `~` to a string (the operator's lenient fallback) rather than a "spread expects a list" error. Acceptable for now; a stricter check can come with S4-era tightening.
