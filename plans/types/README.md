...
Status: **complete** (S0–S3c done, incl. list-building L1–L3; **S4 done** — bounded generics, E0025; **S5 done** — trait coherence, E0027; **S6 done** — `dyn` narrowing `x.as<T>()`, E0028; **S8 done** — declared union types `A | B`; **S7 done** — migration finalize + cleanup, the closing slice)
...
| — | Optional parameters & default values (E0026) — mid-track feature | **done** (see `slice-optional-params.md`) |
| S5 | Trait coherence — overlap/uniqueness (E0027); orphan rule structural | **done** (see `slice-05-trait-coherence.md`) |
| S6 | `dyn` operations + checked narrowing (`x.as<T>()`) | **done** (see `slice-06-dyn-narrowing.md`) |
| S8 | Declared union types ("closed `dyn`") — `A \| B` | **done** (see `slice-08-union-intersection.md`; intersection unbuilt) |
| S7 | Migration finalize + cleanup | **done** (closing slice — see `slice-07-migration-finalize.md`) |

Dependency order is strict-linear S0 → S3c.2 → L1 → L2 → L3 → L4 → S3c.4, then S4 → S5 → S6 → S8 → S7 (S6 may land in parallel with S4/S5 — narrowing is independent of bounds; S8 is gated after S6, since a declared union is a *closed* `dyn` discriminated by the `x.as<T>()` narrowing). S3c grew from a single "hole-elimination" slice once the corpus audit showed a naive hole→error flip is both unsound (rejects valid `return none`) and unfixable today (bindings carried no annotation). The plan then split into propagation (S3c.1) + binding annotations (S3c.2), and a deeper finding followed: the language had *no* list-building at all (immutable lists, no append; `~` only display-concatenated), so the accumulator/backward-inference story was moot. Rather than design the type system around that gap, **L1–L4 add list-building** (concatenation, spread, declaring-scope assignment, compound assignment) so accumulators are both expressible and infer their element type by *forward* inference — which is why no backward-inference solver is needed. `E0023` (S3c.4) then fires only for a genuinely never-constrained binding, fixable via the S3c.2 annotation. Diagnostic codes are append-only; **E0024** is `LoopControlOutsideLoop` (a `break`/`continue` outside any loop — added with the `break`/`continue` feature), **E0025** is `TraitBoundNotSatisfied` (a generic call instantiating a type parameter with a type that does not satisfy its bound — added with S4), **E0026** is `RequiredAfterOptional` (a required parameter following a defaulted one — added with the mid-track optional-parameters feature), **E0027** is `ConflictingTraitImpl` (a type implementing the same trait more than once — added with S5 trait coherence), and **E0028** is `InvalidNarrow` (a checked narrowing `x.as<T>()` applied to an already-concrete, non-`dyn` value — added with S6), so the next free is **E0029**. S8 (declared unions) added **no** new code — a non-member argument reuses `E0007` and an unknown member reuses `E0013`.

## Known gaps (deferred within the track)

Minor checker imprecisions surfaced while building S3c (none broke soundness of accepted programs — they were *missed rejections*, i.e. things that stayed gradual). **All three are now closed** (struck through below); kept here as a record of what was tightened and the narrow false-negatives deliberately left:

- ~~**Method call on a concrete primitive with no such method isn't flagged.**~~ **Closed** (`synth_call` Member arm): a method call on a concrete `int`/`float`/`bool`/`unit` whose result fails to resolve (`Type::Unknown`) now emits `E0007`, exactly mirroring the non-indexable check. `compare` (Comparable, on every value) resolves; `dyn`/holes and user `Named` types stay lenient (a `Named` may resolve via a trait at runtime). Covered by `calling_an_unknown_method_on_a_primitive_is_rejected` + `tests/conformance/types/method_on_primitive.lang`.
- ~~**Spread of a non-list display-concatenates instead of erroring.**~~ **Closed.** The L2 desugar now wraps each spread operand in `..` (`UnaryOp::Spread`) — a runtime-identity marker (`apply_unary` passes the value straight through to the surrounding `~`; the VM `retain`s it since it aliases a live heap reference). The checker requires the operand be a list: a list passes through, a `dyn`/hole spread yields `List<dyn>`, and a concrete non-list is `E0007` (resolving list-shaped so the desugared concat raises no cascading second diagnostic). Covered by `spreading_a_non_list_is_rejected` + `tests/conformance/collections/spread_non_list.lang`.
- ~~**`Ok`/`Err` contextless bindings are not `E0023`; a never-refined `mut []` is not `E0023`.**~~ **Closed** (a focused stand-in for the deferred S3c.3 backward solver — no `Type::Var` machinery needed). (a) `is_uninferable_literal` now also matches `Ok(x)`/`Err(e)`: one constructor fills only one `Result` slot, so an immutable, un-annotated binding to one is always undeterminable → `E0023` (`some(x)`, which fully determines its `Option`, is excluded). (b) A new syntactic pass (`check_unrefined_muts` + `reassigns`/`child_stmt_bodies`) flags a `mut x = []`/`{}`/`none`/`Ok`/`Err` that is *never reassigned* anywhere reachable in its lexical scope (descending `if`/`for`/`fn`/method bodies) — the `mut` exemption exists for accumulators a later write resolves, so with no such write it does not apply. Conservative (an inner shadow's write only ever *suppresses* the diagnostic), and purely syntactic so it needs no type env. Covered by `immutable_context_free_literal_binding_is_e0023` (extended) + `never_reassigned_mut_context_free_literal_is_e0023` + `tests/conformance/bindings/cannot_infer_{ok_constructor,unrefined_mut}.lang`. Still deliberately *not* flagged: a `mut` reassigned only to another open constructor (`mut r = Ok(5); r = Ok(6);` stays `Result<int, ?>`) — a rare false-negative the full solver would catch.
