...
- **Method call on a concrete primitive with no such method isn't flagged.** `(42).foo()` / `n.upper()` where `n: int` silently types as the hole rather than `E0007`. The parallel non-indexable check (`42[0]` → `E0007`, S3b) *is* enforced; the method equivalent is the obvious follow-up (a method call on a concrete `int`/`float`/`bool`/`unit` with no matching built-in method → `E0007`). Natural to fold into S4/S5 or a small checker-hardening pass.
