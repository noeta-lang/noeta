...
Minor checker imprecisions surfaced while building S3c; recorded here so they are not lost (none breaks soundness of accepted programs — they are *missed rejections*, i.e. things that stay gradual):
...
- **`Ok`/`Err` contextless bindings are not `E0023`.** Only `[]`/`{}`/`none` trigger it; `r = Ok(5)` binds at `Result<int, ?>` with an open `Err` slot left gradual (they carry partial info, so the syntactic trigger excludes them). Likewise a never-refined `mut acc = []` stays `List<?>` (mut is exempt from `E0023`). Both are deliberate narrowings that could be revisited if they prove to hide bugs.
