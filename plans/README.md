...
- `<topic>.md` — a **design note** for work that is deliberately not started: the shape of the answer, written down because arriving at it was the expensive part. Each carries a status header saying so and is reachable from the backlog row that owns it, which holds the trigger. Today: `attested-watch-ledger.md`, `fmt-structural-safety-gate.md`, `interruptible-host-io.md`. A note is deleted with the row when the work lands.
