...
| 8 | [Traits as operators + derives + generics](m1/slice-08-traits.md) | B | in progress (8a done: `+ - * / ~` + the `@derive(...)`/`#[...]` decorator surface + E0014/E0015; 8b underway: every operator trait-dispatched — `Equatable` `==`/`!=`, fallible `TryAdd`, `Ordering`+`.compare()`, `Comparable` `< <= > >=`, **`@derive(Comparable)` structural ordering**, **`a[i]` index access via `Index`** (E0016), and the **`@derive` / `#[...]` sigil split** (E0017) all done; todo: `Display`/`ToJson` derive codegen, `Length`/`Iterable`/… protocols, the `Attribute` gate + manifest, generics) |
