...
| 10 | [Layered stdlib (Ring 1 + Ring 2)](m1/slice-10-stdlib.md) | C | in progress (full Ring 1 surface — string/list/map methods + `Set`; Ring 2 `json` via `use std.{json}` — all shared via `lang-stdlib` so the differential holds by construction; `math`/`random` + a harness sandbox for IO/time remain) |
