...
| 8 | [Traits as operators + derives + generics](m1/slice-08-traits.md) | B | in progress (8a done: `+ - * / ~` + `#[derive]` surface + E0014/E0015; 8b underway: `Equatable` `==`/`!=`, fallible `TryAdd`, `Ordering`+`.compare()`, `Comparable` `< <= > >=` all done — every operator now trait-dispatched; todo: derive codegen, other protocols, generics, manifest) |
