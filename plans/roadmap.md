...
| 8 | [Traits as operators + derives + generics](m1/slice-08-traits.md) | B | in progress (8a done: `+ - * / ~` + `#[derive]` surface + E0014/E0015; 8b underway: every operator trait-dispatched — `Equatable` `==`/`!=`, fallible `TryAdd`, `Ordering`+`.compare()`, `Comparable` `< <= > >=`, **`#[derive(Comparable)]` structural ordering**, and **`a[i]` index access via `Index`** (E0016 out-of-bounds) all done; todo: `Display`/`ToJson` derive codegen, `Length`/`Iterable`/… protocols, generics, manifest) |
