...
| 9 | [Modules / namespaces / `use` resolution](m1/slice-09-modules.md) | C | in progress (M1.9.1 done — `lang-loader` multi-file loader + linker resolves `use` to real sibling-module declarations, merged into one program both backends run identically; harness gained multi-file fixtures; M1.9.2 visibility/`pub` + E0013 + cross-module checks and M1.9.3 salsa module graph todo) |
