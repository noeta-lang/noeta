...
| 10 | [Layered stdlib (Ring 1 + Ring 2)](m1/slice-10-stdlib.md) | C | in progress (M1.10.1 + M1.10.2 done — full Ring 1 surface: string methods, list/map methods, `Set` type, all shared via `lang-stdlib` so the differential holds by construction; M1.10.3 Ring 2 modules remain, with a harness sandbox for IO/time) |
