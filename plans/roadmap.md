...
| 8 | [Traits as operators + derives + generics](m1/slice-08-traits.md) | B | in progress (8a done: `+ - * / ~` + `#[derive]` surface + E0014/E0015; 8b underway: every operator trait-dispatched — `Equatable` `==`/`!=`, fallible `TryAdd`, `Ordering`+`.compare()`, `Comparable` `< <= > >=`, and **`#[derive(Comparable)]` structural ordering** all done; todo: `Display`/`ToJson` derive codegen, other protocols, generics, manifest) |
