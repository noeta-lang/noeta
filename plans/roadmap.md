...
| **M2 — Differentiators** | Persistent runtime + isolates; async/structured concurrency + TaskScope; bundled HTTP/WS server; signals; embedded LSP; native toolchain; AOT + DCE; Tier-1 specializing interpreter; HMR; **dev profiler/flamegraph** + **native OTEL instrumentation** (the two halves of what was one "observability" line); agentic MCP surface (incl. semantic role tags); baseline DB; packed value types. | **cluster 1 complete (M2.0–M2.5)** + much delivered since as its own arc: **isolates + channels** (coroutines), **async/`await`/`concurrent`** structured concurrency (coroutines Track A; app-lifetime `TaskScope` DI/workers still deferred), **signals** (reactivity), **embedded LSP + DAP** (editor-tooling), **Tier-1 method-JIT** (Cranelift, off-thread compile, P-JCT), **packed value types** (P-PACK), **http client + Network capability** (http arc). **Remaining:** persistent runtime + bundled HTTP/WS **server** (next up); **AOT + DCE**; native toolchain / package-manager; HMR; agentic MCP surface; baseline DB. **Both observability halves ✅ done** — `noeta profile` (dev flamegraph) + **native OTEL** (`std.telemetry`, production tracing; see split below) |
| **M3 — Long tail** | WASM target (**planned** — `plans/wasm/README.md`: playground + edge, W0–W4); Tauri desktop; background-work extensions; JIT; editor grammars + VS Code ext; reactive persistence; p2p/local-first; extension system + stable host ABI; startup cache; editions; (R&D) checked semantic-edit MCP tool. | not started — **except JIT** (done; Cranelift method-JIT + off-thread compile) and **editor grammars + VS Code ext** (done; TextMate + tree-sitter grammar + LSP/DAP client) |

Detailed M2–M3 decomposition is deferred to a dedicated planning pass when each milestone is reached. M0 and M1 are sliced below. **M2 cluster 1** (host IO & async foundation) is sliced in `m2/`; several later M2 differentiators have since been delivered as their own arcs (see the M2 status cell above). The **genuinely un-started** M2 work: persistent runtime + bundled HTTP/WS **server**, **AOT/DCE** (no build/bundle path exists yet — every `noeta run` recompiles from source; `Module` is not even serializable today — **planned in `plans/aot/README.md`**: bytecode bundle → self-contained exe → native AOT+DCE), native toolchain / package-manager, HMR, agentic MCP surface, and a baseline DB — each awaits its own planning pass. (**Both observability halves are now done** — dev profiler and native OTEL, see below.)
...
- **Dev profiler / flamegraph** — ✅ **DONE** (`noeta profile`, branch `profiler`, plan [`plans/profiler/README.md`](profiler/README.md); not yet merged). A dev-time introspection tool, sibling of DAP/LSP in the dev-tooling cluster (outside the differential like DAP). A payoff on the DAP investment: reuses the call-stack + the **always-emitted line tables** (no debug compile needed — cheaper than expected). Shipped both halves: an *instrumenting* profiler (exact call counts + self/total time) and a *sampling* profiler (wall-time flamegraphs — folded / inferno **SVG** / **speedscope** JSON), plus a **deterministic op-clock** mode (`--every N`), `--lines` leaf attribution, and a top-N summary. Both ride one `Option`-gated per-op seam on the VM (the debugger consult's cheaper twin), tier-0 pinned; **cooperative sampling** (VM snapshots its own stack at a safe point). Naming: `noeta profile` (subcommand ≠ the `--profile` build-tier flag). Tier-1 (JIT-on) sampling deferred (poll at JIT trampoline points).
- **Native OTEL instrumentation** — ✅ **DONE** (branch `native-otel`, plan [`plans/native-otel/README.md`](native-otel/README.md); not yet merged). Production distributed tracing as OpenTelemetry: the `Telemetry` **Host capability** (the 8th), a **hand-rolled OTLP/HTTP-JSON exporter** over the existing reqwest+serde_json (no `opentelemetry-otlp`/tonic/prost), and the `std.telemetry` SDK (`with_span`/`span`/`Span` methods). W3C context propagation both explicit (`current_context`/`span_from`) and **automatic** across `await`s, channels, and isolates — riding on **per-task task-local context** in both backends. **Auto-instrumentation** of `http.serve` requests (SERVER span, inbound-parent, one connected trace per request). Real-host-only + write-only ⇒ never differential-tested; a deterministic sandbox recorder + a byte-identical **span-parity oracle** hold both backends together. **Opt-in** (off until `OTEL_EXPORTER_OTLP_ENDPOINT` is set; `OTEL_SDK_DISABLED` opt-out), behind a default-on `telemetry` build feature. Docs: [Observability](../docs/Observability.md). *Deferred follow-ons:* reactive-flush spans (opt-in), metrics/logs signals. Its "soft-blocked on the bundled server" caveat was lifted by the HTTP-server arc landing first.

The dev-profiler half touched crates then under active refactoring by a parallel effort (the higher-order-abi arc, since merged); native OTEL was built on top of it (its `ExtState`/ctx seam) once it settled.
