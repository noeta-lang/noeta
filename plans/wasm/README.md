...
| W3.1 | async pump | W3.0 | **DEFERRED (recorded, not silent).** Sync XHR covers the synchronous leaf; concurrent `*_async` fan-out degrades serial-but-correct (resolve-at-spawn), same as any host without a real executor. Genuine overlap needs either **JSPI** (wasm stack switching — shipping in Chromium, partial elsewhere) or a steppable VM run loop (an "external pending" state the worker pumps). Revisit when a playground use case actually needs overlapping IO — the seam (`ExternIo`/executor) is unchanged and ready. |
