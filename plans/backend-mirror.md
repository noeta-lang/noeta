...
| JSON deep-marshal (`Value` → neutral `NativeValue` tree) | `noeta-value/src/display.rs` `Value::to_native_deep` ↔ `noeta-eval/src/lib.rs` `value_to_native_deep` (comment-linked twins) | Per-representation glue: the marshalling *policy* (Option-through-payload, enum → variant name, `<fn>` summaries, map sort / object declared order) rides directly on each backend's value walk | KEEP mirrored (error-json arc audit): the encoder itself — the JSON *text* engine serving both `json.stringify` and the `Serialize<Json>` derive's `to_json()` — is already **LIFTED** to `noeta-ext-abi/src/json_text.rs` (audit-1 cross-cutting finding 2), so only this projection is twinned. Divergence-pinned by `std/json_encoder_one_engine.noe` (the two doors byte-for-byte) + the differential |
