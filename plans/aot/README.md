...
| L3.1b | eager AOT compile driver | L3.1a | Compile **every** proto to the object with linkable symbol names + a proto→symbol dispatch manifest. Helpers already resolve as `Linkage::Import` relocations against the runtime crate. Still needs: how a native AOT body reaches another proto's entry (the runtime binds AOT symbols → its jit entry tables at startup instead of JIT-compiling on demand). |
