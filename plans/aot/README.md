...
| L3.2 | link driver → standalone binary + runtime symbol binding | L3.1b + L2.1 | Invoke the system linker (`cc`/`ld` — a build-time toolchain dep, new for this arc; L1/L2 needed none) to combine the object + a prebuilt runtime archive → one executable. At startup the runtime binds the manifest's symbols into its **mutable** per-proto entry tables (`jit_install`) instead of JIT-compiling on demand. **HMR alignment (preserve, do not foreclose):** (a) AOT calls MUST keep routing through the entry-table indirection (`prepare_call`), never baked direct native→native relocations — a future direct-call optimization would be hot-reload-hostile, so gate it behind a non-HMR/release flag; (b) keep the entry tables runtime-mutable and the interpreter/JIT live in HMR builds — a "strip the JIT for size" pure-AOT build forecloses reloading changed protos. |
