...
| L3.2b(3) | link driver + `noeta build --native` + AOT differential | L3.2b(2) | **The remaining linker/build-system step — the ONLY untested variable now.** Needs: a staticlib (`noeta-vm` feature `aot` + a C-ABI `main` that reads the stapled bundle → Module, references `extern static noeta_aot_dispatch`, calls `run_module_aot`); a `cc` invocation `[program.o + libnoeta_aot.a] → exe`, then staple the bundle (L2); `noeta build --native -o app`; and the end-to-end AOT differential (native binary output ≡ `noeta run`). Crux decision deferred: the runtime archive is **built from the workspace** for now (in-tree testable); how it's *packaged/shipped* to end users is a later distribution call. **HMR alignment (preserve):** (a) AOT calls stay indirection-routed, never baked direct native→native — gate any direct-call opt; (b) keep tables mutable + interpreter/JIT live in HMR builds. |
