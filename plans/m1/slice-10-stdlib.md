...
- [ ] **Remaining:** list `sorted` (via `compare_primitive`, homogeneity-checked), `slice` (bounds-checked → `IndexOutOfBounds`), `first`/`last` (→ Option — requires matching the backends' Option construction/rendering); possibly a `Set` type.
