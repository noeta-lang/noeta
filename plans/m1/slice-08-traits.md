...
- [ ] Map/string index receivers (`m[k]`, `s[i]` — need a `map_get` value accessor); the remaining protocols: `Length` (`len`), `Iterable` (`for`), `Callable` (`a(...)`), `Members`/`DynamicCall` — each needs the surface operator routed to its method.
