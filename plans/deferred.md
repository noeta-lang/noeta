...
| Copy-on-write / unique-owner in-place list append (the `~` concat operator copies the whole left `Vec` each time, so an accumulator loop `acc = acc ~ [x]` / `acc ~= [x]` is **O(n²)**) | L1 (list-building) | **Eval-side DONE** (`plans/perf/p-cow-list-append.md`, P-COW) — the binding evaluator takes the old value out of its scope slot before evaluating the RHS of a self-concat `acc = acc ~ rhs` (guarded by "rhs doesn't mention acc"), so a uniquely-owned list (`Rc::get_mut`) is extended in place; an alias keeps refcount > 1 → copy. O(n²)→O(n), 56× at n=8000 (`eval_accumulate` bench). **VM-side still open**, deferred into the `gc-arena` row below: it needs uniqueness info from the heap allocator. Until then the VM stays O(n²) (a perf asymmetry invisible to the differential). Alternatives if COW proves insufficient: a persistent vector (RRB / `im::Vector`, O(log n) with structural sharing) or a mutable `.push` (O(1) but makes lists the second mutable heap type — the change L1 deliberately avoided). |
