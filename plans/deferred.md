...
| **Infer a record/object literal's type arguments from its field values** (`Box { value: 1 }` → `Box<int>`) — a literal currently types as `Box` with *unspecified* arguments (compatible with any instantiation, but not precise). Constructors via an associated `fn` do carry precise arguments. | S4.5 | **TODO** — unify each field value's type against the field's declared parameter to solve the literal's arguments, mirroring `check_generic_call`. Trigger: precise typing of values built by literal rather than constructor. |
