...
| **Struct/class *literal* field values are not type-checked against declared field types** | function-types slice investigation (2026-06-30) | A struct/class literal accepts a field value of **any** type: `struct Box { f: int }` then `Box { f: "hi" }` (or `f: 42` where `f: (int) -> int`) is **accepted**, top-level *and* inside a function body — no `E0007`. Field **defaults** *are* checked (`lang-check/src/lib.rs:1521`, "default value … not assignable to field type"), so the machinery exists; it's simply not applied to literal field values. **Trigger:** "clear typing throughout" — surfaced while adding function types (the function-type *binding/argument* checks fire correctly; only the *literal-field* path is lenient). Fix: in the object/record-literal synthesis arm, check each provided field value against the declared field type (`assignable`, reusing the default-value check), with the same generic-param erasure. Conformance: a positive (correct field types) + a negative (`Box { f: "hi" }` → E0007). |
