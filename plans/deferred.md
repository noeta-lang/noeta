...
| **User `destruct` does not run for an async task's captured locals** (completed *or* cancelled) | A.3a (found during A.8) | Pre-existing: an async fn's locals are hoisted to persistent state-machine cells and released via the plain refcount path (no `__destruct`) when the future is dropped, so a `class` with a destructor created in an `async fn` body never runs it — unlike the same code in a sync fn (which does). No leak (memory is reclaimed; leak oracle residency 0) and both backends agree, so it is a *missing-destructor-call* correctness gap, not a leak. Fix: release an async future's captured cells through the destructor-aware path (or emit explicit `__destruct` calls in the state machine's terminal/abandon states). This is what would make "cooperative cleanup on cancel via `destruct`" actually fire. |
