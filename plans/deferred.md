...
| Redundant checker passes for `type_of` fidelity A (attribute-system P2.3): `lang-compiler::compile` and `lang-eval`'s `run` each call `lang_check::resolve_type_of_sites`, which re-runs the full checker — so a CLI `run` type-checks the program ~3× (the `checked` gate + once per backend). | P2.3 (reflection) | Any perf pass over the front-end. **Approach:** thread a single shared check artifact (the resolved-type map, alongside diagnostics) from one `checked` run into both backends — e.g. a `lang-db` query both `bytecode` and the eval path read — instead of each backend re-deriving it. Functionally a no-op (the map is a pure function of the program); purely removes the duplicate work. Cheap interim guard if needed: skip `resolve_type_of_sites` when the AST contains no `Expr::TypeOf`. |
