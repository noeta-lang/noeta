...
| **Track a type argument through an instance** (`Box<string>` as a value) — `Type::Named` carries no arguments, so an instance loses its `T`: `box.get()` types as `dyn` rather than `string`, and a bound that only constrains a method's receiver-shaped use is not enforced at instance-method calls. Construction enforcement (S4.3b) already guarantees a constructed instance's `T` satisfies its bounds, so this is about *precise typing*, not soundness. | S4.3b | **TODO (S4.5)** — needs `Type::Named(name, args)` (≈30 lattice/checker sites, no runtime change) + binding class parameters from the receiver's arguments at method calls. Trigger: precise return typing of generic containers / instance-method bound enforcement. |
