...
**Status: scoped, not started.** This is the reconnaissance-backed scope for the built-in dev
...
|---|-------|----------|-------|
| **P0** | Crate + `noeta profile` + tier-0 profiled run | `noeta profile app.noe` runs a program to completion under the profiler harness and prints total wall time + op count | New `noeta-prof` crate (deps mirror `noeta-dap`) + `Command::Profile`/`cmd_profile()`; session = ordinary compile, `jit=false` run. **Naming + tier-0 + crate-shape decisions land here.** No collector yet. |
| **P1** | Instrumenting profiler (counts + self/total) | Per-function table: calls, self ms, total ms, self% | `Option`-gated call-boundary seam on `Vm` (call-count `Vec<u64>` + enter/exit ns; self = total − children). Labels via `Chunk.name`. `--instrument`, sorted text table + `--format json`. **Exact fixture tests** (recursive `fib`, a hot inner fn) assert counts exact and self-time ordering. |
| **P2** | Sampling profiler (wall-time flamegraph) | Folded stacks the program spends time under | Timer thread → `AtomicU32` pending; op-boundary poll walks `frames` → interned stack aggregator → collapse to folded stacks. Default mode of `noeta profile`. **`--every Nops` deterministic op-clock mode** lands here too, so a fixture asserts the hot function dominates folded output **exactly**. `--format folded`. |
| **P3** | Rendering + line attribution | An SVG flamegraph / speedscope profile you can open; leaf-line labels | `inferno` folded→SVG (`--format svg`, default for sampling); speedscope JSON (`--format speedscope`); `-o <file>`. Line-granular leaf via `line_span`+`line_col` (`file:line` in frame labels); instrument gains per-line self-time within a function. Terminal top-N summary. |
| **P4** | Docs + roadmap + memory | The arc is recorded | New `docs/Profiling.md` wiki page; `docs/The-CLI.md` command table row; cross-link from `docs/Performance-Techniques.md` / `Editor-and-AI-Tooling.md`; roadmap tick (profiler half of observability done); `plans/done/` move; memory entry. |
