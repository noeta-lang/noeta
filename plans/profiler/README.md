...
**Status: P0–P3 done (P3 rendering; line-attr + top-N deferred); P4 next.** This is the reconnaissance-backed scope for the built-in dev
...
| **P3** ✅ (rendering) | Rendering formats | An SVG flamegraph / speedscope profile you can open | **DONE (formats).** `render` module + `Format {folded,svg,speedscope,table,json}`; `--format <fmt>` + `-o <file>` (artifact→file, else stderr — program keeps stdout). `inferno` (0.12, `default-features=false`) folded→**SVG**; **speedscope** JSON (`type: sampled`, shared frame table + weighted samples, opens at speedscope.app); instrument **`--format json`**. Format/mode mismatch (e.g. `svg` on `--instrument`) and unknown format both exit 2 before running. **6 fixtures** (svg contains `<svg`+hot fn; speedscope valid schema, in-range frame indices, endValue==total; json exact `fib`=177; format parse). fmt/clippy clean, **zero unsafe** (speedscope interns with owned keys). **Deferred (surfaced, not silently cut):** *line-level attribution* (leaf `file:line` in folded frames / per-line self-time — changes folded labels, so opt-in) and the *terminal top-N summary* — small follow-ups, tracked as P3.1. |
