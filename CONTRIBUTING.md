...
4. Keep it green and clean with `scripts/gate.sh`, which runs what `.github/workflows/ci.yml` runs and prints a per-step PASS/FAIL summary:
   ...
   - `scripts/gate.sh` — the merge gate: adds the workspace suite, the lean-CLI and feature-shape builds, the doc samples, the JIT oracles, the `#[ignore]`d real-socket suites that drive a live `noeta serve --watch` through a hot swap, and the instructions-retired perf ratchet (~20 min warm, 42 min cold). **Run it, green, before merging to `main`.**
   - The perf ratchet (`scripts/perf-ratchet.sh`) pins startup and interpreter cost in *instructions retired*, because wall-clock cannot gate a box running several builds at once — a 2x startup regression once survived ~1,800 commits for exactly that reason. It fails on an improvement too, so the floor ratchets down: re-record with `scripts/perf-ratchet.sh --record` and commit `tests/perf/baseline.txt` with your change, reading the diff's sign as you go. It needs `perf`; without it the gate SKIPs loudly rather than passing.
