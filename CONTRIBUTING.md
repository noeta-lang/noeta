...
4. Keep it green and clean with `scripts/gate.sh`, which runs what `.github/workflows/ci.yml` runs and prints a per-step PASS/FAIL summary:
   ...
   - `scripts/gate.sh` — the merge gate: adds the workspace suite, the lean-CLI and feature-shape builds, the doc samples, the JIT oracles, and the `#[ignore]`d real-socket suites that drive a live `noeta serve --watch` through a hot swap (~15 min warm, 35 min cold). **Run it, green, before merging to `main`.**
