...
- **A hook, not a channel and not a `Notify`.** The wake a blocked party needs is whatever its own blocking primitive understands, and `noeta-vm` — which owns the cancel — has neither tokio nor a host. `RealExecutor` registers `notify_one()` on a per-executor `tokio::sync::Notify` that every point `advance` can block now `select!`s on (the timer sleep and the task join alike); a future blocking host leaf would register a condvar's `notify_all` instead, and the VM stays ignorant of both.
...
The same select covers the executor's *other* block — waiting on the `JoinSet` for whichever async IO leaf finishes first — so a worker awaiting `fs.read_async` or `p.read_line_async()` is roused too. That does **not** finish the job for a blocking leaf, and the reason is worth stating because it is easy to assume away: ending the wait is not ending the work. The leaf's body is a `spawn_blocking` closure on the isolate's own tokio runtime, and dropping a runtime waits for every blocking task that has already started — so the woken worker unwinds, and then its teardown blocks until the leaf returns anyway. Measured both ways: at the unit level a 2 s blocking body is woken past in 50 ms and then costs the drop the remaining 1.95 s; at the CLI, a worker awaiting `fs.read_async` on a FIFO with no writer, cancelled at 300 ms, hangs past 20 s — and unwedging the FIFO at 900 ms ends the run at 903 ms. Interrupting host IO for real therefore needs each blocking leaf to *return* an `Interrupted` outcome, not merely to be abandoned; the design is in `plans/interruptible-host-io.md`, and the synchronous host calls the paragraph above names have the same requirement plus one more — that thread is inside `RealHost`, not the executor, and has registered no hook at all.
