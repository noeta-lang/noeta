...
A **union** orders when its members are *mutually comparable*: each one orders on its own, and any two of them can be compared with each other. Numbers are the family that crosses — every numeric type compares against every other — so [`number`](Type-System#number--any-numeric-scalar) and any union of numeric members is an ordered `T`, and `[3, 1.5, 2u8].sorted()` under a `List<number>` annotation sorts by value across the widths. Every other ordering is keyed to one type, so a union that mixes families has none: `int | string` and `Uuid | Instant` are refused even though each member orders alone, and a union holding an unordered member (`int | List<int>`) is refused for the plainer reason. Both refusals name the union, and both reach `<` and `.compare()` alike — the operator and the method are one rule, so a union either orders at both or at neither. Narrowing is the way through: inside `if x is int { … }` the value is an `int`.
