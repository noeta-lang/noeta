...
- The composites a generic body can carry a width through are the ones its own **parameter types** name, sub-terms included — `fn f<T>(xss: List<List<T>>)` carries `List<List<T>>` and `List<T>`. That is where the values a body can pass on come from. One assembled inside the body out of a bare parameter (`wrap([v])` in `fn f<T>(v: T)`) names an instantiation no caller of `f` could have supplied, so it reads the erased word. Three further positions carry no instantiation either, and read it for the same reason a `dyn` does: a **generic type's method** (whose parameter rides the receiver), a `dyn` local inside a generic body, and a generic function taken as a value with nothing to pin its instantiation.
