...
- Two positions carry no instantiation at all and so read the erased word: a `dyn` local inside a generic body, and a generic function taken as a value with nothing to pin its instantiation. Nothing there is refused — a width only ever refines an answer the erased word already gives.
