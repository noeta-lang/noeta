...
- **Displaying** a value reads its static type, so a `u64` above `i64::MAX` prints its unsigned value — at `echo`, in an interpolation hole, in a `~` concatenation, at the REPL's echo of a trailing bare expression, and inside a list, set, map, tuple, object field or enum payload rendered from a `u64`-typed position. Once a value is laundered through `dyn` the width is gone (exactly as it is for a type test), and the erased 64-bit word prints as a signed `int` — as it does at a `noeta repl --no-check` prompt, which runs no checker and so has no static type to read.
- **Ordering** a value reads its static type too, so a `u64` above `i64::MAX` sorts above every smaller value: `.sorted()`, `.min()`, `.max()`, a rendered set or map, `.keys()`, `.values()`, and a `for` over a set or map all place it by its value. Inside a `dyn` position the width is gone, and those doors fall back to the erased word's order.
...
- The composites a generic body can carry a width through are the ones its own **parameter types** name, sub-terms included — `fn f<T>(xss: List<List<T>>)` carries `List<List<T>>` and `List<T>`. That is where the values a body can pass on come from. Three positions carry no instantiation and read the erased word for the same reason a `dyn` does: one assembled inside the body out of a bare parameter (`wrap([v])` in `fn f<T>(v: T)`), which names an instantiation no caller of `f` could have supplied; a `dyn` local inside a generic body; and a generic function taken as a value with nothing to pin its instantiation. Nothing there is refused — a width only ever refines an answer the erased word already gives.
