...
The overflow series aggregates the same way the instrument does, so what folding costs depends on the instrument. A counter or up/down counter keeps its total. A histogram keeps its count, its sum and its bucket distribution, over the same buckets as every other series. A **gauge** is last-value, so folded measurements overwrite each other and the overflow point reports only the most recent one — a gauge has no total to keep, which is a reason to be stricter about attribute values on gauges than anywhere else.
