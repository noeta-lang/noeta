...
**One thing a bounded test gives up:** its hot *loops* stay in tier 0. Being stoppable means staying somewhere with safepoints, and JIT-compiled native code has none — so a run that can be cancelled declines on-stack replacement (the promotion path that takes a loop over mid-flight) while keeping ordinary call-count promotion. Most tests never notice: a loop whose body touches the heap or calls out already returns to the interpreter every iteration, so it was never going to run natively anyway. A loop the JIT *can* sustain end to end pays for it — measured, a 200-million-iteration counting loop takes 7.0 s under a bound and 0.8 s without one.

The trade is narrower than that number makes it sound, because the tests that pay it are exactly the tests that would otherwise be unstoppable: a loop fast enough to go fully native is a loop with no safepoint in it, which is the one shape a timeout could not do anything about. If you have a test that genuinely wants both, `#[Timeout(0)]` opts it out of the bound and back into full tiering. Benchmarks are unaffected — `noeta bench` does not run under the rail.
