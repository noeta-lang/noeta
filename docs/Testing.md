...
Almost every overrunning test stops. A test that is *running* — spinning in a loop, recursing, grinding through work — reaches a safepoint within an iteration, unwinds from there, and tears its isolate down exactly as a finished test does: its destructors run, its heap goes back to zero, and anything it spawned is cancelled and joined with it. A test that is **not** running — parked in a long `sleep`, waiting on a child that has stopped talking or has not exited, waiting on a request or a stream that has gone quiet — stops too: the request rouses it out of that wait, and it unwinds from the safepoint it wakes into. Its thread is then joined, so nothing is left behind at all. Two things follow that are worth knowing:
...
One class cannot be stopped: **a test blocked in a file read the operating system will not return from** — a FIFO with no writer, a character device. That thread is not executing Noeta, no safepoint comes around, and unlike the other waits there is nothing to rouse: the wait belongs to the kernel. Its thread is abandoned, it keeps running until the process exits, and everything its isolate owns is held until then. The report says so explicitly rather than leaving you to guess, because it changes what you do about it: the fix is a deadline on the *operation* — the read's own timeout — not a bigger bound on the test around it.
