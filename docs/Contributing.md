...
- **The skipped-destructor oracle** — the third way precise reference counting goes wrong, and the one neither residency nor the differential can see: an object whose type declares `destruct` is reclaimed by a release that never asked whether a destructor was due. The memory comes back, residency returns to zero, and the program simply prints one line fewer than it should — a *wrong answer* produced by a *correct* free. The differential is blind for a different reason: both backends read the same IR to decide where drops go, so a missing drop is missing in both and they agree byte for byte on the wrong output. The oracle counts objects allocated with a destructor-bearing shape against destructors actually run; since residency is asserted zero in the same measurement, every object allocated was freed, so a surplus allocation is precisely an object freed with its `destruct` skipped. Inert until armed (one thread-local bool per shaped allocation), and runs inside the leak and JIT oracles alongside the anomaly count. It reads the VM heap, so — like the anomaly count — it is a VM-side measurement; and it is corpus-only, because the fuzz generator emits no `destruct` blocks for it to weigh.
...
- **The real-host corpus gate** (`cargo test -p noeta-cli --test conformance_real_host`) — every async corpus case run through the shipped `noeta` binary and held to its own `// expect:` header. Every other corpus oracle runs on `SandboxHost` + `SandboxExecutor`, where `advance` *jumps* the clock to exactly the next deadline, so `sleep(1)` and `sleep(2)` can never come due at the same poll; the AOT differential is the one arm on a real host, and it compares its two real runs to *each other*, never to the header. Between them sits a case whose output holds only under logical time — real, and it reached CI: on a real host the executor sleeps real time to the earliest deadline and wakes late, every deadline the overshoot crossed comes due at the same poll, and the scheduler resumes those tasks in spawn order. A corpus case therefore states its timing in gaps that survive a loaded scheduler, and the ones that genuinely cannot — a real OS thread, a capability the sandbox host stubs — are listed with a reason that the gate prints on every run.
...
- **The docs oracles** (`cargo test -p noeta-cli --test doc_samples`) — two, because a page has two halves a reader copies. Every ` ```noeta ` block runs through the real binary (untagged must exit 0, `check` must type-check, `error` must fail, `ignore` opts out), and every ` ```toml ` block must parse as a manifest the toolchain accepts. The second exists because the manifest parser is strict enough to be worth asking: it refuses a table the schema does not define and a directive provider that names no dependency, which is how a page came to declare `crit = "acme/criterion:bench"` two lines under its own sentence saying that is an error. `[package]` is optional, so a fragment (`[directives]` alone) is checked as written; tag ` ```toml ignore ` for a block that is deliberately not a `noeta.toml` — a `Cargo.toml`, a Spin manifest, a `noeta.lock` excerpt.
