...
- **Shipped (http-server arc):** the **inbound** side of the Network capability (`net_listen`/`net_accept` as an async leaf/`net_reply`), a `Request` extern type, and the `http.serve(port, handler)` construct + `noeta serve` command — a concurrent HTTP server (the sandbox drives a deterministic request script, the real host binds a `TcpListener`). Notably, `http.serve` is **not** a registry function: the handler is a closure the `NativeValue` seam cannot carry and the loop needs the executor + inbound Network capability, so it is a hardcoded `Builtin` — the same shape as `signal`/`computed`/`effect`/`all`/`race`/`map_bounded`. Making such **higher-order / effectful native functions** (and CLI commands) registrable through the ABI — so this whole family + `noeta serve` migrate out of core into a dogfooded `std` extension — is a recorded follow-on (see Deferred).
- **Deferred:** **higher-order / effectful native functions + extension commands** — the registry `DispatchFn` can carry only pure/host functions (data in, data out); it cannot take a **closure argument**, call it back, or reach the **executor/reactive graph**, and the CLI has no **extension-command** seam. So every function needing those is a hardcoded `Builtin` (`signal`/`computed`/`effect`/`all`/`race`/`map_bounded`/`serve`) and `noeta serve` is a core command. Building those seams migrates the whole family out of core into a dogfooded `std` extension. Also deferred: columnar kernels via extensions (blocked on a raw-buffer ABI capability), Host-coupled finalizers (GC free cannot reach the Host — buffered types keep explicit `close()`), and the package/dependency manager that would let `vec`/`quat` physically leave core and third parties register their own crates. That milestone is where the ABI gets *frozen/versioned* (a published contract) and the static std-only registry becomes a dynamic multi-extension registry — the crate boundary already exists (P-NATIVE); freezing it does not.
