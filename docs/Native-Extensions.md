...
[dependencies]
noeta-native = { git = "https://github.com/…/noeta", tag = "vX" }
```

The subtlety is a Rust one: a type's identity includes *which compiled copy of the crate* it came from, so if `noeta-native` is compiled twice — once for the shim, once for the git entry crate — its `Extension` trait exists **twice** as two unrelated types, and the shim's `units.extend_from_slice(ext0::NOETA_EXTENSIONS)` no longer type-checks (a `dyn Extension` from one copy doesn't satisfy the other). The whole graph must resolve `noeta-native` to **one** source. Two cases:
...
- **The consumer runs a workspace (local-path) toolchain** — the development norm, and while iterating on the toolchain itself. Now the shim's `noeta-native` is a *path* and the package's is *git* — two sources. The composer closes this by injecting a **`[patch]`** into the shim that rewrites every `crates/*` member of the noeta repo to the consumer's exact path, so the package's git-deps (and their transitive `workspace = true` deps) all collapse onto the one local copy. The patch key is the toolchain's own `repository`, overridable with **`NOETA_TOOLCHAIN_REPO`** for a fork, a private mirror, or a local `file://` clone (it must equal the URL the package's `Cargo.toml` declares). Cargo *does* fetch the git source before applying the patch, so the toolchain repo must be reachable — fine for a public repo.

So a first-party-but-out-of-tree package (the `para` family) uses the git-dep form; the same `[patch]` machinery serves any third-party native package that git-depends `noeta-native`. In-tree packages keep a path dep and never touch any of this.
...
All host-coupled effects — filesystem, clock, PRNG, `env`/`args`, the operating system (`os`: subprocess exec + spawn/lifecycle control + system introspection), entropy, ids, the network, and the three telemetry signals — go through one `Host` trait (eleven mandatory capability traits, blanket-impl'd), plus one **optional** capability, `P2pProvider`: a host either offers peer networking (`as_p2p` → its `P2p` impl) or declines it (the default `None`). Two implementations exist: `SandboxHost` (deterministic in-memory VFS, logical clock, seeded RNG, a **pure network responder**, and a scripted exec command set — what the differential always runs) and `RealHost` (real disk, real env, real subprocesses, per-isolate tokio, and a real reqwest client — what `noeta run` uses, never differential-tested).

**P2p is the first capability an *extension* provides rather than the host.** When the p2p stack left `std` for the non-default `para` package, `P2p` stopped being a mandatory arm of `Host` (that is what `P2pProvider` optional-izes) — and its deterministic loopback broker moved out of the hosts entirely into the `para.p2p` extension, which owns one in per-run ctx state (`ExtState`) and reaches it through a small `with_p2p` seam: the host's real transport when it offers one (`RealHost`'s p2panda node), otherwise the extension's own broker. So the three loopback hosts (sandbox, WASI, browser) implement no `P2p` at all, and the capability *travels with the package*. The one wrinkle the seam solves: the async `p2p.receive` leaf is `Send` while `ExtState` is not, so the broker lives behind a `Send` handle (`Arc<Mutex<…>>`) the receive descriptor captures at spawn — the ABI that lets an extension own an async-reachable host capability. This is the same "simulate deterministically, deploy real" split as the async executor and isolate scheduler. The network capability (http arc) set the async pattern: `RealHost` overrides `net_spawn` to hand the executor a genuine `RealBody::Async` reqwest future while the sandbox resolves at spawn; `os.exec_async` follows it with a `RealBody::Blocking` subprocess body.
...
- **Shipped (para-namespace arc):** the first first-party capability to leave `std` for a **non-default package** under a new namespace (`para`). The HTML liveview moved to the pure-Noeta `para.html` package (`packages/para-html/`), and the p2p/local-first stack (`crdt`/`p2p`/`synced`) was physically extracted from `noeta-stdlib` into the `noeta-para-p2p` crate + the native `para-p2p` package (`ParaP2pExtension`, root `para`), installed only when a program depends on it and authorizes it in `[trust]`. `noeta-crdt` and the `ring-p2p` transport stay put; the AOT footprint scan still links the p2panda tree only when `para.p2p`/`para.synced` are imported. **Reactive seam (a contract to stabilize):** `para.synced` is a node in the *same* reactive graph as core `std.reactive`, so `noeta-stdlib::reactive` exposes `ReactiveExt`/`STATE_KEY`/`state_of`/`sync_gates`/`drive_flush` and a `register_view_source_extractor` hook (so core `view.expose` accepts a foreign node type without naming it) — the one-way dependency that lets the p2p surface live out of `std`. This seam is the one-way dependency that lets the p2p surface live out of `std`.
- **Shipped (para out-of-tree follow-on):** the toolchain groundwork for taking a first-party package fully out-of-tree. **Editions** — `[package] edition` is now a validated, per-package pin recorded in `noeta.lock` and folded into the compiled-bytecode cache key (one edition today; the seam for future language/ABI evolution). **`git` deps track a `branch` or HEAD**, not only a tag, so an in-development or bundled package needs no cut release (the lock still pins the resolved SHA). **The P2p capability is decoupled from the hosts** — `P2p` is optional (`P2pProvider`), and its loopback broker is owned by the `para.p2p` extension in ctx state, reached through the `Send`-handle receive ABI (see [The `Host` capability](#the-host-capability)) — the three loopback hosts drop their `P2p` impl entirely. **Out-of-tree native packages build** — the composed toolchain injects a `[patch]` (`NOETA_TOOLCHAIN_REPO`-configurable) that unifies a package's git-referenced toolchain crates onto the consumer's copy, proven end-to-end for `para-p2p`; the pure-source `para-html` split (standalone repo → registry round-trip) is proven too. In-tree copies stay path deps until the toolchain/registry repos are published (the remaining step).
...
- **Deferred:** the **hosted** registry + publishing the toolchain repo — the step that makes a package's true departure from the monorepo portable (a `file://` clone proves the mechanism today, but a committed git-dep needs a reachable public repo). The out-of-tree *build* path (git-deps + the composer `[patch]`) is in place; what remains is publishing. Also deferred: dynamic loading (the dyn dispatch tables are already in place; every compiled-in extension monomorphizes past them).
