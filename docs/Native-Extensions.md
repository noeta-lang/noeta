...
The whole former `Builtin` family is the dogfood: `task.sleep`/`all`/`race`/`map_bounded` (drive loops over `call`/`poll`), `http.serve` (the accept→dispatch→reply loop, including the recover-from-abort pattern: a handler abort becomes a 500 and the loop continues), and all of `std.reactive`.
