...
The first-party but non-default packages live under the `para` namespace in their own repositories (github.com/noeta-lang/para-*) and are published on the hosted registry at [registry.noeta.dev](https://registry.noeta.dev): `para/html` (LiveView), `para/api` (HTTP middleware, mocking, pagination), `para/db` + `para/aether_db` (database drivers and stores), `para/cli`, `para/aether` (the web framework), and `para/p2p` (the local-first / peer-to-peer stack). Add one to `[dependencies]` — several of the same scope bind under one key as a scope array, `para = [ { version = "^0.1", package = "para/aether" }, … ]` — and authorize a native one in `[trust]`; see [Using Packages](Using-Packages). Each repo documents its own surface.
