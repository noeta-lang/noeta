...
- **Agentic DX surface** (architecture §12) — the differentiator for a training-data-less language, mostly *exposing existing models* through one interface:
  ...
  - **Semantic role tags** (architecture §12.7): a `SemanticRole` trait lets a framework's attribute confer a typed architectural `Role` (entry point, trust boundary, persistence boundary, sink, layer, …) into the manifest, declared once and inherited by every use; the compiler indexes `(declaration, Role)` at build time and MCP tools (`list_roles`/`trace_from`/`trace_to`/`flows_between`) query the resulting semantically-labeled call/use graph in architectural terms ("trace this entry point to every persistence write," "does any trust boundary reach a sink"). Pure *composition* over the attribute manifest (§9.13), the salsa call graph (§9.17/§6), and the MCP server (§12.4) — small (a trait, a typed enum, a manifest-indexing pass, a few query tools), so it **sequences after** those three exist. The role index also feeds architectural-conformance lints (§9.17) for free.
...
- **(R&D) Checked semantic-edit MCP tool** (architecture §12.8) — borrow the staleness-safe, node-targeted edit discipline of graph-as-source systems *on top of* the text-authoritative core: an MCP edit tool that targets a semantic node with an expected current value, guarded by a graph/state hash, computes the corresponding source edit, and re-checks with the compiler before commit. Prototype against the `check_snippet` baseline (§12.5) and only adopt if it demonstrably beats it. Gated on the hard open problem — stable node-identity over text edits — and on the salsa graph (§6), manifest (§9.13), MCP server (§12.4), and `fmt`/CST machinery all existing first; evaluated after the core agentic DX, never before. Text stays authoritative; if a prototype drifts toward a canonical graph store it has left this direction.
