...
- An attribute may confer a typed architectural **role** on every declaration it annotates with a `@role(Role)` directive naming one variant of the blessed `Role` enum (`EntryPoint`, `PersistenceBoundary`, `TrustBoundary`, `Sink`, `Layer`); the compiler indexes `(declaration, Role)` at manifest-build time, surfaced to user code by `roles_of()` → `List<RoleBinding>`. Like placement, `@role` is a static declarative tag, *not* a user-evaluated `fn role(): Role` (same comptime avoidance); only a record marked `@attribute` may carry it (else E0031), and a parameterized role (`Layer(name)`, `Custom(name)`) — evaluated per use site — is deferred as a comptime enhancement.
