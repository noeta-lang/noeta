...
Code generation and data attributes are **two distinct operations with two distinct sigils**, so neither overloads the other: **`@derive(...)`** is compile-time codegen (the compiler synthesizes trait impls from a type's fields; a closed, compiler-provided set), while **`#[...]`** attaches a **data attribute** (a record instance, discovered via the manifest and acted on by a consumer; an open, user-definable set). The one-line model: `@` = the compiler generates something; `#[...]` = metadata attached (the PHP-attributes model). A third concept, hand-written `impl Trait for Type { ... }`, is unchanged.
...
**Roles ride attributes; they are not a separate annotation.** A framework's attribute carries a declarative `@role(Enum.Variant)` directive naming the role it confers (`@role(Semantic.EntryPoint) type Route = { path: string };`). The role is declared **once** on the attribute and **inherited by every use** — so an application that already annotates its handlers with `#[Route]` gets a queryable semantic graph for free, with no per-call-site tagging. Only a record marked `@attribute` (§9.13) may carry `@role`, since the role is conferred on whatever the attribute attaches to (a class/enum cannot — attributes are records only; otherwise E0031). A declaration may carry several `@role` tags (each its own binding).
