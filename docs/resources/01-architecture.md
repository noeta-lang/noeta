...
- **Packed value types:** a `struct` whose fields are all primitives (or other packed value types) is laid out **unboxed and contiguous** — no header, no shape, value semantics, passed by value. This is what makes `Vec3`, `Quat`, `Mat4`, colors, and similar SIMD-amenable. It is a natural extension of the existing **struct** type (§4.1): structs are already value-semantics and structural; packed value types add a predictable-layout guarantee for the all-primitive case. Structural update and `Clone` (§4.2) already fit value-copy semantics.
...
There is no special constructor. The single creation *primitive* is the **all-fields struct literal** (`Money { amount: ..., currency: ... }`), which the compiler requires to assign **every** field — this is the one syntactic choke point where full initialization is guaranteed before an object can escape. Everything else builds on it:
...
- **No promotion needed.** Fields are declared once in the struct/class body and filled by the literal, so PHP 8's constructor-promotion ergonomics come for free rather than as a special signature form.
...
| `__invoke` | `Callable` | `a(...)` |
| `JsonSerializable` | `Serialize<Json>` | serialization |
...
| `__serialize`/`__unserialize` | `Serialize<Format>` | (de)serialization |
...
- **Derivation kills boilerplate.** `@derive(Equatable, Comparable, Display)` synthesizes field-wise implementations for the common value-object case; hand-written `impl` is reserved for custom logic. (Rust `#[derive]` / Swift automatic synthesis.) Codegen has its own `@` sigil, kept distinct from the `#[...]` data attributes of §9.13.
...
Code generation and data attributes are **two distinct operations with two distinct sigils**, so neither overloads the other: **`@derive(...)`** is compile-time codegen (the compiler synthesizes trait impls from a type's fields; a closed, compiler-provided set), while **`#[...]`** attaches a **data attribute** (a struct instance, discovered via the manifest and acted on by a consumer; an open, user-definable set). The one-line model: `@` = the compiler generates something; `#[...]` = metadata attached (the PHP-attributes model). A third concept, hand-written `impl Trait for Type { ... }`, is unchanged.
...
- **Built-in derives (compile-time, implemented inside the compiler).** The shape-based codegen cases — `Serialize<Json>`, `Equatable`, `Comparable`, `Clone`, ORM-hydrate, etc. — are standard derives shipped by the language/stdlib and implemented as ordinary compiler code in Rust, where they are well-tested and produce good diagnostics. A derive may be **generic** (`Serialize<Format>` is parameterized by its serialization format). Users *apply* `@derive(Serialize<Json>)` but cannot *write new* derives. This covers the overwhelming majority of real "reflection for codegen" needs at zero runtime cost and with no user-facing metaprogramming. User-defined derives are **deliberately out of scope**, deferred unless they prove necessary; if ever added, they would be a narrow, constrained mechanism (typed `TypeInfo` in, code through a restricted builder out), never open-ended comptime.
- **Opt-in runtime reflection (for the genuinely dynamic minority).** Plugin systems loading types by name, generic debuggers, `new` from config. Redesigned to be cleaner than PHP's: **unified with the real type system** (`type_of(value)` returns the *same* `Type` the checker uses, not a parallel `ReflectionType` hierarchy), **introspection separated from invocation** (read-only `Type` introspection vs. explicitly **fallible** invocation — `type.construct(args)` and `method.call(obj, args)` return `Result<_, _>` because calling-by-name can fail on arity/types), **pattern-matchable** (`match type_of(value) { Type.Struct(r) => ..., Type.Enum(e) => ... }` instead of an `instanceof ReflectionClass` ladder), and **capability-gated** (a type is reflectable only if marked or reflected-upon, so unused metadata is eliminated by dead-code analysis — this reconciles runtime reflection with small AOT binaries; reflectable types become tree-shaking roots, §9.8.1). Critically, runtime reflection is **closed-world**: it only ever touches the finite set of types that already exist in the program, so it does *not* require the compiler at runtime and does *not* block tree-shaking — it stays in the default static build tier. It is categorically distinct from `eval` (which creates new code from strings, opening the world); see §9.8.1. The "dynamic minority" here means dynamic *dispatch over known types*, not dynamic *code creation*.

**Attributes are just structs used in annotation position — no special construct.** This is a deliberate consistency choice: rather than invent a bespoke `attribute` declaration (a new language concept, and one *less* capable than the types the language already has), an attribute is an ordinary **struct** (§4.1) constructed in annotation position. `#[Route("/users")]` constructs `Route { path: "/users" }` through the exact same constructor-as-ordinary-function machinery (§4.1) as any other value — all-fields-literal checking applies, and a validating constructor returning `Result` even gives fallible attributes, which PHP cannot express. This reuses machinery that already exists instead of adding a parallel one, consistent with the language's "express things in the real language, not a mini-language" discipline (the same reason operators are traits §9.2 and lint rules are functions §9.17).

The attribute model (§9.2's "everything is a struct/trait" applied here):
- **Attributes are structs, marked with the `@attribute` directive.** A struct opts in with `@attribute`; the `#[Foo(...)]` arguments then construct it by mapping onto its fields (positional in declaration order, named by name) — the one unambiguous construction a struct has. They are structs (not classes) precisely *because* of this: a class has only convention-named constructors, with no canonical one for the attribute system to call. A struct can still gain *behavior* via a standalone `impl SomeTrait for Route {}`, so "structs only" costs no expressiveness.
- An attribute that constrains *where* it may be attached lists the permitted declaration kinds in the directive — `@attribute(Method, Function, …)` over (`Struct`, `Class`, `Enum`, `Function`, `Method`, `Field`, `Variant`), enforced by the checker at compile time (E0030); a bare `@attribute` attaches anywhere. This is a static, declarative list — deliberately *not* a user-evaluated `valid_target(t): bool` predicate, which would require compile-time execution of user code (the project avoids comptime). A predicate over a target's *return type* would be a future enhancement gated on comptime.
- An attribute may confer a typed architectural **role** on every declaration it annotates with a `@role(Enum.Variant)` directive naming a fieldless variant of a `@semantic` enum — the built-in `Semantic` (`EntryPoint`, `PersistenceBoundary`, `TrustBoundary`, `Sink`, `Layer`) or any project enum marked `@semantic`; the compiler indexes `(declaration, role)` at manifest-build time, surfaced to user code by `roles_of()` → `List<RoleBinding>` (each `role` typed as the abstract `Enum` kind). Like placement, `@role` is a static declarative tag, *not* a user-evaluated `fn role(): Enum` (same comptime avoidance); only a struct marked `@attribute` may carry it (else E0031), multiple roles per declaration are allowed, and a parameterized role (`Layer(name)`) — evaluated per use site — is deferred as a comptime enhancement.

```
@attribute(Method)                     // opt-in + constrain where it attaches
...
**Roles ride attributes; they are not a separate annotation.** A framework's attribute carries a declarative `@role(Enum.Variant)` directive naming the role it confers (`@role(Semantic.EntryPoint) struct Route { path: string }`). The role is declared **once** on the attribute and **inherited by every use** — so an application that already annotates its handlers with `#[Route]` gets a queryable semantic graph for free, with no per-call-site tagging. Only a struct marked `@attribute` (§9.13) may carry `@role`, since the role is conferred on whatever the attribute attaches to (a class/enum cannot — attributes are structs only; otherwise E0031). A declaration may carry several `@role` tags (each its own binding).

**The vocabulary is a typed enum, not free-form strings — and it is user-extensible.** The language ships a built-in `Semantic` enum with a blessed core set — `EntryPoint`, `PersistenceBoundary`, `TrustBoundary`, `Sink`, `Layer` — and any project enum marked `@semantic` joins the same role namespace (so a web framework defines `@semantic enum WebRole { Controller, … }` and tags with `@role(WebRole.Controller)`). The value of the feature is a *shared* vocabulary every agent and tool understands uniformly; promoting an enum with `@semantic` keeps that vocabulary typed and matchable rather than a free-form tag-soup, while still letting frameworks extend it. `roles_of()` surfaces each binding's role as the abstract `Enum` kind (§ kind-types), since a binding's role may be any `@semantic` enum.

**Compile-time only, and declarative.** `@role` is a fixed `Enum.Variant` name (the named variant must be **fieldless**), recorded by the compiler as a `(declaration, role)` index alongside the existing `(declaration, attribute)` index — a static build artifact, never runtime reflection, with zero runtime cost. It is deliberately *not* a user-evaluated `fn role(): Enum` reading the attribute's fields: that would require compile-time execution of user code (comptime), which the project avoids exactly as for attribute placement. A *parameterized* role (`Layer(name)`), whose payload would be evaluated per use site, is the one piece deferred to a future comptime capability.
...
### 12.8 Authoring model: a text-authoritative core, with checked semantic editing as an R&D direction

**Decided: the language is text-authoritative.** Source text *is* the program. The semantic graph — the salsa query graph (§6), the attribute manifest (§9.13), the semantic role index (§12.7) — is a *queryable derivation* of that text, never the source of truth. Agents author ordinary source text, verified through the compiler-as-oracle (§12.5), and *query* the semantic model through the MCP server (§12.4). This is a deliberate contrast to **graph-as-source** designs (e.g. Vercel Labs' "Zero"), where a compiler-owned graph is authoritative and text is a generated projection edited through a bespoke node-targeted patch protocol. The bet: LLMs are trained to predict text, so they are strongest *writing code-like text* — meeting them there and layering a semantic query/verification surface on top is lower-friction than asking them to author in a graph-patch protocol for which no training data exists. Graph-as-source buys surgical, staleness-checked edits at the cost of training-data alignment and human-native authorability; for a general-purpose, human-co-equal language that trade is judged not worth it. This is the same coherence principle as the rest of §12 — the semantic model is a consumer-facing *derivation* of the existing spine, not a parallel authoritative store.

**R&D (flagged bet, not committed): a checked semantic-edit MCP tool.** The genuinely good primitive inside graph-as-source systems is the *edit discipline*, and that is borrowable without making the graph authoritative: an edit targets a specific semantic node, carries an **expected current value**, and is guarded by a **graph/state hash** so a stale or invalid edit fails before it lands. The direction is an optional MCP *semantic-edit* tool complementing the §12.4 semantic-*query* tools — an agent says "change this node, which I expect currently to be X; reject if the graph has shifted since I read it," the tool computes the corresponding **source** edit (text stays authoritative), applies it, and re-checks with the compiler before commit. Agents that want staleness-safety and surgical precision get it; the source of truth stays text. It *complements*, never replaces, the propose→verify→commit baseline (write text + `check_snippet`, §12.5).

**Why it is R&D and not a milestone.** Three open questions keep it a bet. (1) **Node identity over text edits** — graph-as-source gets stable node IDs for free; a text-authoritative model must derive stable identities from the salsa/CST structure so "this expression" stays addressable as the surrounding text changes (the hardest open question). (2) **Round-tripping edits to text** — applying a semantic edit means computing the minimal, correctly-formatted source change via the `fmt`/CST machinery, so the node→source-span mapping must be exact. (3) **Whether it beats the baseline** — whether this materially outperforms "write text + `check_snippet`" for real agentic workflows is empirical and must be measured before any commitment. Guardrails if it is ever prototyped: do **not** let the graph become authoritative (the tool computes a *text* edit; a prototype that drifts toward a canonical graph store has become the rejected design); prototype-measure-then-decide, per project discipline; and reuse existing machinery (node addressing from salsa/CST, edit application via `fmt`/CST, validation via the existing checker) — assembly over the §12 spine, not a new subsystem. It depends on the salsa graph (§6), the manifest (§9.13), the MCP server (§12.4), the `fmt`/CST machinery, and the oracle baseline (§12.5) all existing first, so it is evaluated *after* the core agentic DX is in place — an M3 / R&D item, never before.

### 12.9 Coherence of the DX story
