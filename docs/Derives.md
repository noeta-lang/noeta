...
| `Comparable` | Field-wise ordering, in declaration order (recurses into nested objects and enum payloads). On an **enum**: variant declaration order first (`Low < Medium < High`), then payload fields. What `< <= > >=`, `.sorted()` and `.min()`/`.max()` all use — deriving it is how a type opts into being ordered at all. |
