...
| **Project diagnostics** | The whole workspace, not only the files you have open: the server answers `workspace/diagnostic` by running the same project walk `noeta check` runs, with your unsaved buffers overlaid on the files on disk. A fault in a module nobody has opened reaches the problems panel. It is a *pull* — the client asks for it on idle, not on every keystroke — so the per-edit path stays as narrow as it was. |
| **Hover** | What the cursor is on decides what you get. A **callable's name** shows its whole declaration (`fn add(a: int, b: int): int`) — at a call site as well as at the declaration, so you read the parameters rather than the call's result type alone. A **type name** shows its declaration: fields or variants, and method signatures. An **embedded-language tier** name (`@sql { … }`) describes its body — the declared language, and for an expression tier its value type. A **decorator directive** (`@role`, `@packed`, `@derive`, …) is described in place. Inside a **`use`**, both imported items and module path segments resolve, and a **namespace group** (`http` from `use std.http`) lists its members. Anything else is the inferred static type of the expression, in surface syntax (`List<int>`, `Result<Order, OrderError>`). Non-default storage adds a fact line: a `@packed` type shows `@packed — 12 bytes`, a `List<packed>` shows `flat packed storage — 12 bytes/element, row-major` (or `column-major (SoA)` for `@packed(Layout.Column)`). Whatever matches, the declaration's `@doc` prose follows after a rule. |
...
- `check` — type-check code; the same JSON diagnostics `noeta check --format json` emits, over the same shapes: once as the source ships, then once per dev-tier block it declares, with `tiers_checked` naming which. A `@test` body that does not compile is an error here, not a surprise at `noeta test`. `file` takes a **project directory** as readily as a single `.noe`: it is literally the same walk `noeta check` runs, so the agent and the command line cannot disagree about whether a project is clean.
