...
The authored name is the last dotted segment of `id.name`. Every path a tool reports is relative to the project root.

### What the graph holds

The graph tools read the program the entry links, which holds what the entry imports and nothing else. `symbols(scope: "workspace")` reads every module's own source, so it is the larger set: a module's `@test` block is referenced by nothing and is outlined there while `trace`, `callers`, `impact` and `path` cannot reach it.

Each outline node says which it is with `in_graph`. A graph tool asked for a name it cannot reach names the declaration, its file and its tier when the sources hold one, so the answer tells a name outside the graph from a name that does not exist.
...
| `code_search` | The **project's own declarations**, ranked against a name (`place_order`), a qualified path (`orders.place_order`), or a plain sentence (`where does an order get written to the database`). It indexes every declaration of the linked workspace, including what a `@test`/`@bench` block declares, over its name, qualified path, `@doc` prose, signature, `@role` bindings and attributes, file path, and the identifiers its body mentions. `matched_fields` names the fields that earned each hit, `kind` and `roles` narrow the set, and every result carries an `id` the graph tools accept. |
...
| `symbols` | A declaration outline, carrying each node's `@role` bindings, its `tier` for a declaration written inside a `@test`/`@bench` block, and `in_graph`. `scope: "workspace"` outlines every module of the project; the default `scope: "file"` outlines the entry alone. |
...
`trace`, `reflect` and `module_graph` read the merged workspace program, and each reports `linked` with `link_diagnostics` in `check`'s JSON shape. Under a failed link the answer comes from the entry file's own parse, where names lose their qualification, so `trace` marks every node `unverified` and a callee naming one of the project's own modules has an `id.kind` of `unresolved`.
...
| `trace` | The **static call path from a role or a function**. `from` takes a role (`EntryPoint`, with every bearer as a root) or a function name, qualified (`App.Lib.add`) or bare as `symbols` reports it (`add`, `Counter.bump`); an ambiguous name returns `candidates`, an unknown one the closest declarations. Each node is a function with its roles, declaration and call sites. External calls and dynamic callees are labeled leaves, a passed-function reference such as a handler registration is a `reference` edge, and a function reached a second time is marked `shared`. The `boundaries` summary lists the persistence and trust boundaries the roots reach. |
...
| `impact` | What changing a declaration reaches: the reverse transitive closure over the call graph. Address it by `symbol`, or by an edit (`edit_file` plus `new_source`, diffed in memory; nothing is written). Returns the impacted declarations with their ids and the `@test`/`@bench` functions among them. When a module's top-level statements use one of them, that module is a node of the answer and `attributed` is `false` with the reason, since a test runner reruns everything the top level reaches. |
| `path` | **How one declaration reaches another**: the `k` shortest chains of calls and passed references from `from` to `to` (default 3, max 10), each listing its nodes with ids, the edge kind of every hop, and each hop's call site. A route may end on an external or dynamic callee and never passes through one. Routes come back weakest first and strongest last; `ranker: "shortest"` orders them by hop count. |
| `context_map` | **The declarations worth reading around a set of seeds, under a token budget.** Seeds are declaration names, files, module paths, or a `@role`, which seeds every bearer. The call graph and the `use` import graph are ranked from the seeds with personalized PageRank, and the strongest connected declarations are emitted as signatures grouped by file until `budget_tokens` runs out (default 4096). Each node carries its `id`, `rank`, `score`, and the `via` edge that reached it. `ranker` takes `degree` or `random` for measuring the default against them. |
| `architecture` | **The project as its role graph**: one node per `@role` bound anywhere in it, one edge per pair of roles, aggregated from the call graph with every declaration bearing no role collapsed out. Each edge carries the number of declaration-to-declaration connections behind it and a few of them by name, and an edge reached only through passed references is marked. `connections` holds the bearer-level graph the aggregation was taken over, and declarations bearing no role are counted as `unassigned` with the most-connected named. Read it before `trace`, which unfolds one entry point of the same graph. |
