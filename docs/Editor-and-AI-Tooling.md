...
- **Editor grammars and a VS Code extension** — the VS Code grammar now exists (above); still to come are a tree-sitter grammar (for Neovim/Zed/Helix) and folding the LSP client into the same extension (M3).
...
- For **VS Code**, install the bundled extension in `editors/vscode-noeta/` for proper `.noe` highlighting. Other editors have no bundled grammar yet; their generic highlighting handles the C-family surface reasonably until the tree-sitter grammar lands.
