...
| `architecture` | **The project as its role graph**: one node per `@role` bound anywhere in it, one edge per pair of roles, aggregated from the call graph with every declaration bearing no role collapsed out of the edges. Each edge carries how many declaration-to-declaration connections it stands for and a few of them by name, and an edge reached only through passed references is marked. Reports every `(declaration, role)` binding, and for declarations bearing no role at all, how many there are and the most-connected of them. Read it before `trace`, which unfolds one entry point of the same graph. |
