...
The id is the **one** spelling of a node's name, kind, file and span: no tool repeats them beside it. Read the authored name as the last dotted segment of `id.name`. Every path a tool reports, `id.file` and a navigation target alike, is relative to the project root.
...
Each outline node says which it is with `in_graph`. A graph tool asked for a name it cannot reach names the declaration, its file and its tier when the sources hold one, so a name outside the graph reads differently from a name that does not exist.
