...
Within a declaration, a name belongs to one member of its kind. Two fields called `x`, two variants called `A`, or two methods called `f` in one body is **E0079**, in every kind — `struct`, `class`, `enum` and `trait` alike:

```noeta error
struct Point {
    x: int
    y: int
    x: int      // E0079: `Point` declares field `x` more than once
}
```
...
pub struct PetStore {
    // E0079 if the spec's servers already give `PetStore` a `base_url`.
